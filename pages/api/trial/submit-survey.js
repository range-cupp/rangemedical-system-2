// /api/trial/submit-survey
// Saves pre or post trial survey responses
// Updates trial_passes survey completion flags
// On post-survey: runs check-in recommendation engine
// Range Medical

import { createClient } from '@supabase/supabase-js';
import { generateTrialRecommendation } from '../../../lib/trial-checkin';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (surveyType === 'pre') {
      ...
      // Run check-in recommendation engine
      ...
      const postSurvey = { energy, brain_fog, recovery, sleep, stress, noticed_notes };

      const result = generateTrialRecommendation({
        preSurvey: preSurvey || {},
        postSurvey,
    ...
    console.log(`Trial survey (${surveyType}) saved for trial ${trialId}`);
