...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber }) {
  try {
    ...
    // ── PEPTIDE: Staff-managed only ──
    // NO auto-creation, NO auto-extension. Task Damon to create/extend protocol.
    if (protocolType === 'peptide') {
      const existing = await findExistingProtocol(patientId, protocolType, medication);
      const protocolId = existing?.id || null;
      ...
      // Parse peptide details for the task
      const catalogMatch = findPeptideProduct(serviceName);
      const peptideName = catalogMatch?.medication || medication || serviceName;
      const duration = parseDurationFromName(serviceName) || (catalogMatch?.duration) || 30;
      const peptideInfo = catalogMatch ? findPeptideInfo(catalogMatch.medication) : findPeptideInfo(medication);
      const frequency = peptideInfo?.frequency || catalogMatch?.frequency || '';
      const dose = peptideInfo?.startingDose || catalogMatch?.dose || '';

      const DAMON_ID = '8a62c77f-5c18-4b59-9e1e-70113ab4954e';
      const todayStr = new Date().toISOString().split('T')[0];
      const taskTitle = existing
        ? `Peptide: Extend protocol for ${patientName || 'patient'} — ${peptideName}`
        : `Peptide: Create protocol for ${patientName || 'patient'} — ${peptideName}`;
      const taskDesc = [
        ...
        dose ? `Dose: ${dose}` : null,
        '',
        existing ? 'Extend the existing protocol.' : 'Create a new peptide protocol in the service log.',
      ].filter(Boolean).join('\n');

      await supabase.from('tasks').insert({
        title: taskTitle,
        description: taskDesc,
        assigned_to: DAMON_ID,
        assigned_by: DAMON_ID,
        patient_id: patientId,
        patient_name: patientName,
        priority: 'high',
        status: 'pending',
        ...
        if (tErr) console.error('Auto-protocol: peptide task create error:', tErr);
      });

      console.log(`Auto-protocol: peptide purchase — linked to ${protocolId || 'none'}, tasked Damon (${peptideName}, ${duration}d)`);
      return;
    }
