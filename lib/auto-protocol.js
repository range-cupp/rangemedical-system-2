...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber }) {
  try {
    ...
    if (existing && protocolType === 'weight_loss') {
      // Weight loss: do NOT auto-extend — staff will update the protocol manually
      // Still log the pickup/delivery so there's a record of the purchase
      protocolId = existing.id;
      const wlQtyMatch = (serviceName || '').match(/x(\d+)/i);
      const isMonthlyProgram = (serviceName || '').toLowerCase().includes('monthly');
      const wlInjections = wlQtyMatch ? parseInt(wlQtyMatch[1]) : (isMonthlyProgram ? 4 : (quantity || 1));
      const wlDoseMatch = (serviceName || '').match(/(\d+(?:\.\d+)?)\s*mg/i);
      const wlDose = wlDoseMatch ? `${wlDoseMatch[1]}mg` : (existing.selected_dose || null);
      const wlDosageLabel = wlInjections > 1 ? `${wlInjections} week supply @ ${wlDose || ''}` : wlDose;
      const wlIsInClinic = fulfillmentMethod === 'in_clinic_injections';
      ...
      if (!wlIsInClinic) {
        await supabase.from('service_logs').insert({
          ...
          dosage: wlDosageLabel,
          quantity: wlInjections,
          fulfillment_method: fulfillmentMethod || 'in_clinic',
          tracking_number: trackingNumber || null,
          notes: `Auto-logged from purchase: ${wlInjections} injection${wlInjections > 1 ? 's' : ''} dispensed (protocol not auto-extended — update manually)`,
        });
      }
      console.log(`Auto-protocol: weight_loss repurchase for patient ${patientId} — logged delivery, skipped auto-extend (staff will update protocol)`);
    } else if (existing) {
