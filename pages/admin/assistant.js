...
export default function AssistantPage() {
  ...
  async function openPatientProfile() {
    if (!patient?.id || profileLoading) return;
    setProfileOpen(true);
    setProfileLoading(true);
    try {
      const [recordsRes, consentsRes, paymentsRes] = await Promise.all([
        fetch(`/api/ai/patient-records?patient_id=${patient.id}`),
        fetch(`/api/ai/consent-status?patient_id=${patient.id}`),
        fetch(`/api/ai/payment-history?patient_id=${patient.id}`),
      ]);
      const [records, consents, payments] = await Promise.all([recordsRes.json(), consentsRes.json(), paymentsRes.json()]);
      setProfileData({ ...records, consents, payments, patient_id: patient.id });
    } catch {
      setProfileData(null);
    }
          ...
          <div style={st.patientBar}>
            ...
            {patient ? (
              <div style={st.selectedPatient}>
                <User size={14} />
                <span style={{ fontWeight: 600, cursor: 'pointer', textDecoration: 'underline', textDecorationColor: '#c7d2fe', textUnderlineOffset: '2px' }} onClick={openPatientProfile}>{patient.name}</span>
