...
export default function AppMessages() {
  ...
  const handleCallPatient = () => {
    ...
    voice.call({ to: activeThread.phone, name: activeThread.name });
  };
  ...
  const sendMessage = async () => {
    ...
    if ((!hasText && !hasImage) || !activeThread || sending) return;
    ...
    try {
      await fetch('/api/app/messages', {
        ...
        body: JSON.stringify({
          ...
          message: body || (uploadedMediaUrl ? '📷 Image' : ''),
  ...
  // Helper to parse and render media images
      ...
      <>
        ...
        <div className="thread-shell">
          <div className="thread-header">
            <button
              onClick={() => { setActiveThread(null); setMessages([]); setShowEmojiPicker(false); setImagePreview(null); fetchConversations(); }}
          ...
          <div className="thread-compose">
            ...
            <button className="compose-action" onClick={() => setShowEmojiPicker(!showEmojiPicker)} title="Emoji">
              😊
            </button>
            <button className="compose-action" onClick={() => imageInputRef.current?.click()} title="Send image">
              📷
            </button>
            <input
              ref={imageInputRef}
            ...
            <textarea
              ...
              rows={1}
            />
            <button className="send-btn" onClick={sendMessage} disabled={(!draft.trim() && !imagePreview) || sending || uploadingImage}>
              {uploadingImage ? (
        ...
        {/* Floating call bar for thread view */}
        <AppCallBar
