    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
        ...
        <tr><td style="padding:32px 28px;">
          <h2 style="margin:0 0 16px;font-size:24px;color:#111;font-weight:800;">Last Call &mdash; Ends Tonight</h2>
          <p style="margin:0 0 20px;font-size:15px;color:#444;line-height:1.7;">${greeting} just a quick reminder &mdash; our Mother's Day Wellness Credit ends tonight:</p>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#fafafa;border:1px solid #e0e0e0;margin:0 0 20px;">
            ...
            <tr>
              <td style="padding:12px 20px;font-size:14px;color:#666;">Gift it or keep it for yourself</td>
            </tr>
          </table>
          ...
          <p style="margin:24px 0 0;font-size:13px;color:#999;text-align:center;">Ends tonight. Non-refundable. Max 2 per person.</p>
        </td></tr>
...
export default async function handler(req, res) {
  ...
  try {
    const { data: purchasedEmails } = await supabase
    ...
    if (dryRun) {
      return res.status(200).json({
        ...
        purchased_excluded: purchasedSet.size,
        sample: toSend.slice(0, 5).map(p => ({ name: p.name, email: p.email }))
      });
    }
      ...
      try {
        ...
        const resp = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            to: email,
            subject: "Last Call — Mother's Day Wellness Credit Ends Tonight",
            html,
          })
        });
        ...
        if (resp.ok) {
          ...
          await logComm({
            channel: 'email',
            messageType: 'mothers_day_reminder_blast',
            message: "Mother's Day Wellness Credit reminder — ends tonight",
            source: 'mothers-day/reminder-blast',
    ...
    console.log(`Mother's Day reminder blast: ${stats.sent} sent, ${stats.failed} failed out of ${toSend.length}`);
    ...
    console.error("Mother's Day reminder blast error:", error);
