...
export function getProtocolTracking(protocol) {
  ...
  if (isWeightLoss) {
    ...
    const rawExhausted = totalSessions > 0 && sessionsUsed >= totalSessions;

    // Priority 1: next_expected_date (set by dispense system — most accurate)
    // When next_expected_date is in the future, the patient has been refilled —
    // sessions_used/total_sessions may not have been updated yet, so don't treat as exhausted
    if (protocol.next_expected_date) {
      const nextDate = new Date(protocol.next_expected_date + 'T00:00:00');
      const daysUntil = Math.ceil((nextDate - today) / (1000 * 60 * 60 * 24));
      // If next refill date is still in the future, patient is supplied — not exhausted
      const sessionsExhausted = rawExhausted && daysUntil <= 0;
    ...
    if (isTakeHome && totalSessions > 0) {
      ...
      if (refillDate) {
        ...
        // If supply window is still open, not exhausted
        const sessionsExhausted = rawExhausted && daysRemaining <= 0;
