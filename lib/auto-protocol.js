...
function parseSessionCount(serviceName) {
  const match = (serviceName || '').match(/(\d+)(?:[-\s]Sessions?)?[-\s]?Pack/i);
...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}, fulfillmentOpts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'nad_injection': {
      ...
      const nadTakeHome = nadSessions > 1;
      const endDate = new Date();
      ...
      return {
        ...
        sessions_used: nadTakeHome ? nadSessions : 0,
        status: nadTakeHome ? 'completed' : 'active',
        delivery_method: nadTakeHome ? 'take_home' : 'in_clinic',
        end_date: endDateStr,
        next_expected_date: nadTakeHome ? endDateStr : calculateNextExpectedDate({ protocolType: 'nad_injection', startDate: today }),
      };
    }
