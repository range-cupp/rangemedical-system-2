...
const STAFF = {
  'burgess@range-medical.com': {
    displayName: 'Damien Burgess FNP',
    aliases: ['Dr. Burgess', 'Damien Burgess', 'Dr. Damien Burgess'],
    title: 'Provider',
  ...
  'brendyn@range-medical.com': {
    displayName: 'Brendyn Reed FNP',
    aliases: ['Brendyn', 'Brendyn Reed'],
    title: 'FNP',
    role: 'provider',       // NP — clinical/provider tone
    isAdmin: false,
    isNoteAuthor: true,
...
// Check if an identifier (email, display name, or alias) can author/sign notes.
// Handles both email-based currentUser (session.user.email) and display-name-based
// currentUser (user_metadata.full_name) so the sign/lock UI works regardless.
export function canUserAuthorNotes(identifier) {
  if (!identifier) return false;
  const lower = identifier.toLowerCase();
  for (const email of NOTE_AUTHORS) {
    if (lower === email) return true;
    if (lower.includes(email)) return true;
    const aliases = AUTHOR_ALIASES[email] || [];
    if (aliases.some(a => lower === a)) return true;
  }
  return false;
}
...
export const CONTROLLED_DISPENSE_STAFF = [
  ...
  'brendyn@range-medical.com',   // Brendyn Reed FNP
...
// Order matters — first entry is the default approver in the Dose Change modal.
export const DOSE_APPROVAL_STAFF = [
  'burgess@range-medical.com',   // Damien Burgess FNP (default)
  'brendyn@range-medical.com',   // Brendyn Reed FNP
];
