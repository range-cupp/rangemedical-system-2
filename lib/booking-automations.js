...
export async function runBookingAutomations({ appointmentId, calcomBookingId, eventTypeSlug, serviceCategory, patientId, patientName, patientEmail, patientPhone, serviceName, startTime }) {
  // Skip all booking automations for backdated appointments — staff manually
  // logging past visits shouldn't trigger prep instructions, intake forms, or
  // blood work alerts (the visit already happened).
  if (startTime && new Date(startTime).getTime() < Date.now() - 60 * 1000) {
    console.log(`Skipping booking automations for ${patientName}: appointment start_time is in the past (${startTime})`);
    return;
  }
  ...
  // T-02: Send prep instructions — DISABLED 2026-05-04 at user request.
  // TODO(after step 2 of Cal.com cutover): walk through every service's
  // prep SMS/email with the user, refresh the copy, then turn this back
  // on — likely sourcing the body from the new service_prep_instructions
  // table (admin-editable per service) instead of the hardcoded
  // PREP_INSTRUCTIONS map in lib/appointment-services.js. The current
  // PREP_INSTRUCTIONS list (and the backfilled service_prep_instructions
  // rows) are the starting point.
