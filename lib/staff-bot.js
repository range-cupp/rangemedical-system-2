...
function regexFallback(text, todayISO) {
  ...
  // check_availability
  if (/\b(availab|open slot|any slot|is there.*slot|slot.*available)\b/.test(t)) {
...
async function parseIntent(text, staffName) {
  ...
  if (quick && quick.intent !== 'unknown') {
    // For simple intents that don't need entity extraction, use the regex result directly
    // For complex ones (add_note, create_task, book_appointment with missing fields), prefer Claude
    const simpleIntents = ['get_schedule', 'help'];
    if (simpleIntents.includes(quick.intent)) return quick;
  }
