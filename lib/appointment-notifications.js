...
import { createLabPrepToken, buildLabPrepUrl } from './lab-prep-token';
...
function buildBloodDrawSMS(firstName, date, time, gender, type, labPrepUrl) {
  const base = type === 'reschedule'
    ? `Hi ${firstName}, your appointment with Range Medical has been rescheduled to ${date} at ${time}.`
    : `Hi ${firstName}! Your appointment with Range Medical is confirmed for ${date} at ${time}.`;

  const prepUrl = labPrepUrl || 'https://www.range-medical.com/lab-prep';

  return `${base} Please review your prep instructions and confirm you've read them:\n\n${prepUrl}\n\n— Range Medical`;
}

const SMS_TEMPLATES = {
  confirmation: ({ firstName, serviceName, date, time, isBloodDraw, gender, labPrepUrl }) =>
    isBloodDraw
      ? buildBloodDrawSMS(firstName, date, time, gender, 'confirmation', labPrepUrl)
      : `Hi ${firstName}! Your appointment with Range Medical is confirmed for ${date} at ${time}. See you there!`,
  cancellation: ({ firstName, serviceName, date }) =>
    `Hi ${firstName}, your appointment with Range Medical on ${date} has been cancelled. Call (949) 997-3988 to reschedule.`,
  reschedule: ({ firstName, serviceName, date, time, isBloodDraw, gender, labPrepUrl }) =>
    isBloodDraw
      ? buildBloodDrawSMS(firstName, date, time, gender, 'reschedule', labPrepUrl)
      : `Hi ${firstName}, your appointment with Range Medical has been rescheduled to ${date} at ${time}. See you there!`,
};
...
const EMAIL_SUBJECTS = {
  confirmation: () => 'Appointment Confirmed — Range Medical',
  cancellation: () => 'Appointment Cancelled — Range Medical',
  reschedule: () => 'Appointment Rescheduled — Range Medical',
};
...
async function sendSMS(to, message, patientId = null) {
  ...
  const result = await sendSMSRouter({ to: normalizedTo, message, skipEmailCopy: true });
  return {
...
export async function sendAppointmentNotification({ type, patient, appointment }) {
  // Skip notifications for backdated appointments (start_time already in the past).
  // Staff frequently log past visits manually — we don't want to text patients
  // about appointments that already happened.
  if (appointment.startTime && new Date(appointment.startTime).getTime() < Date.now() - 60 * 1000) {
    console.log(`Skipping ${type} notification for ${patient.name}: appointment start_time is in the past (${appointment.startTime})`);
    return;
  }
  ...
  const isBloodDraw = BLOOD_DRAW_SLUGS.includes(appointment.serviceSlug);

  // Look up patient gender for blood draw appointments (gender-specific prep instructions)
  let gender = null;
  if (isBloodDraw && patient.id) {
    const { data: patientData } = await supabase
      .from('patients')
      .select('gender')
      .eq('id', patient.id)
      .single();
    gender = patientData?.gender?.toLowerCase() || null;
  }

  // Generate lab prep acknowledgment token for blood draw appointments
  let labPrepUrl = null;
  if (isBloodDraw && type !== 'cancellation') {
    const phone = await resolvePhone(patient);
    const appointmentDate = appointment.startTime ? appointment.startTime.split('T')[0] : null;
  ...
  // Dedup check for live sends: if this exact appointment slot already
  // got a confirmation in the last 10 minutes, skip — prevents duplicates
  // from a quick double-click or duplicate POST. Scoped to the specific
  // appointment startTime so back-to-back bookings for different visits
  // each get their own notification.
  if (!quiet && patient.id) {
    const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
    const { data: recentlySent } = await supabase
      ...
      .eq('message_type', messageType)
      .eq('status', 'sent')
      .eq('metadata->>startTime', appointment.startTime || '')
      .gte('created_at', tenMinutesAgo)
      .limit(1);
    if (recentlySent && recentlySent.length > 0) {
      console.log(`Dedup: skipping duplicate ${messageType} for ${patient.name} on ${appointment.startTime} (sent within 10 min)`);
      return;
    }
  }
