 ...
 */
export function buildAdaptiveHRTSchedule(startDate, firstFollowupWeeks = 8, bloodDrawLogs = [], labs = [], labProtocols = [], rescheduleLogs = []) {
  if (!startDate) return [];
  ...
  const MIN_DRAWS = 5; // initial + 4 follow-ups (standard HRT program)
  const MAX_DRAWS = 50; // safety cap to prevent infinite loops

  // Build reschedule override map: drawLabel → new target date
  const rescheduleMap = {};
  for (const log of rescheduleLogs) {
    if (log.notes && log.log_date) {
      rescheduleMap[log.notes] = new Date(log.log_date + 'T00:00:00');
    }
  }
  ...
  const completedLabStatuses = ['awaiting_results', 'uploaded', 'under_review', 'ready_to_schedule', 'consult_scheduled', 'in_treatment', 'blood_draw_complete', 'results_received', 'provider_reviewed', 'consult_complete'];
  ...
  // Label weeks are generated dynamically as we add draws
  function getLabelWeek(index) {
    if (index === 0) return 0;
    return firstFollowupWeeks + ((index - 1) * 12);
  }
  ...
  for (let drawIndex = 0; drawIndex < MAX_DRAWS; drawIndex++) {
    ...
    // Label for this draw
    const label = drawIndex === 0 ? 'Initial Labs' : `${getLabelWeek(drawIndex)}-Week Labs`;

    // Apply reschedule override if one exists for this draw
    if (rescheduleMap[label]) {
      targetDate = new Date(rescheduleMap[label]);
    }
    ...
    // Priority 4 (Initial Labs only): the patient's most recent lab BEFORE the
    // HRT protocol started, within 12 months. Initial workup labs are routinely
    // drawn weeks-to-months before HRT actually begins (lab review consult,
    // scheduling, etc.) — without this, every patient who labs'd up in advance
    // shows "Initial Labs overdue" forever. Bounded to 365 days so labs from
    // a totally separate prior episode don't get incorrectly attributed.
    if (!matchedDate && drawIndex === 0) {
      const PRE_START_WINDOW_MS = 365 * 24 * 60 * 60 * 1000;
      let bestDate = null;
      let bestIdx = -1;
      let bestLogId = null;
      for (let i = 0; i < allDrawDates.length; i++) {
        const entry = allDrawDates[i];
        if (entry.consumed) continue;
        if (entry.date >= targetDate) continue; // must be BEFORE start
        const gap = targetDate - entry.date;
        if (gap > PRE_START_WINDOW_MS) continue;
        // Pick the lab CLOSEST to the protocol start (i.e. most recent pre-start lab)
        if (!bestDate || entry.date > bestDate) {
          bestDate = entry.date;
          bestLogId = entry.logId;
          bestIdx = i;
        }
      }
      if (bestDate) {
        matchedDate = bestDate;
        matchedLogId = bestLogId;
        matchedIdx = bestIdx;
      }
    }
    ...
    schedule.push({
      ...
      rescheduled: !!rescheduleMap[label],
    });

    // Once we have the minimum draws AND an upcoming draw, stop generating
    if (drawIndex >= MIN_DRAWS - 1 && hasUpcoming) break;
  }
