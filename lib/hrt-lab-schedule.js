...
export function matchDrawsToLogs(schedule, bloodDrawLogs = [], labs = []) {
  ...
  for (const log of bloodDrawLogs) {
    if (log.log_date) {
      allDrawDates.push(new Date(log.log_date + 'T00:00:00'));
    }
  }
  ...
  for (const lab of labs) {
    ...
    if (labDate) {
      allDrawDates.push(new Date(labDate + 'T00:00:00'));
    }
  }
  ...
  return schedule.map(draw => {
    ...
    // Find the closest matching draw date within the window
    let matchedDate = null;
    let closestDiff = Infinity;

    for (const drawDate of allDrawDates) {
      const diff = Math.abs(drawDate - target);
      if (diff <= windowMs && diff < closestDiff) {
        closestDiff = diff;
        matchedDate = drawDate;
      }
    }
