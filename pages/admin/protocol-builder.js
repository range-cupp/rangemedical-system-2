...
export default function ProtocolBuilder() {
  ...
  // ── Peptide grouping — collapse multi-duration items under one expandable row ──

  // Derive base name: "Selank — 10 Day" → "Selank", "BPC-157 / TB-4 — 30 Day" → "BPC-157 / TB-4"
  const getBaseName = (name) => {
    const match = name.match(/^(.+?)\s*[—–-]\s*(\d+\s*Day|\d+\s*Week)/i);
    return match ? match[1].trim() : null;
  };

  // Group items within a subcategory by base name
  const groupItemsByName = (items) => {
    ...
    for (const item of items) {
      ...
      if (base && !seen.has(base)) {
        const variants = items.filter(i => getBaseName(i.name) === base);
        if (variants.length > 1) {
          seen.add(base);
          groups.push({ type: 'group', baseName: base, items: variants, color: getCategoryColor(variants[0].category), priceRange: `${formatPrice(Math.min(...variants.map(v => v.priceCents)))}–${formatPrice(Math.max(...variants.map(v => v.priceCents)))}`, description: variants[0].description });
          continue;
        }
      }
      if (base && seen.has(base)) continue; // already grouped
  ...
  const renderGroupedCatalog = (items) => {
    const grouped = groupItemsByName(items);
    return grouped.map(entry => {
      if (entry.type === 'single') return renderCatalogCard(entry.item);
      const isOpen = expandedGroups[entry.baseName];
      return (
        <div key={entry.baseName} style={{ marginBottom: '8px' }}>
          <div
            onClick={() => toggleGroup(entry.baseName)}
            style={{
              border: '1px solid #e0e0e0',
            ...
            onMouseLeave={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
          >
            <div style={{ color: isOpen ? '#1a1a1a' : '#ccc', transition: 'color 0.15s', flexShrink: 0, display: 'flex' }}>
              {isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '14px', fontWeight: '700', color: '#1a1a1a', lineHeight: '1.3' }}>{entry.baseName}</div>
              <div style={{ fontSize: '12px', color: '#737373', lineHeight: '1.4' }}>{entry.description}</div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: '12px', fontWeight: '600', color: '#737373' }}>{entry.priceRange}</div>
              <div style={{ fontSize: '10px', fontWeight: '500', color: '#a0a0a0', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
          ...
          {isOpen && (
            <div style={{ borderLeft: `3px solid ${entry.color}`, marginLeft: '0', paddingLeft: '0', background: '#fafafa' }}>
              {entry.items.map(item => (
                ...
                >
                  <div style={{ color: '#ccc', flexShrink: 0 }}><GripVertical size={12} /></div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: '#1a1a1a' }}>
                      {item.name.replace(entry.baseName, '').replace(/^\s*[—–-]\s*/, '') || item.durationLabel}
                    </div>
                  </div>
                  <div style={{ fontSize: '13px', fontWeight: '700', color: '#1a1a1a', flexShrink: 0 }}>
                    {formatPrice(item.priceCents)}
                  </div>
                  <button style={{ ...s.catCardAdd, width: '24px', height: '24px' }} onClick={(e) => { e.stopPropagation(); addToPlan(item.id); }} title="Add to plan">
                    <Plus size={12} />
                  </button>
                </div>
              ))}
      ...
      <div style={s.page}>
        ...
        <div style={s.catalog}>
          ...
          <div style={s.catalogList}>
            ...
            {/* Peptides grouped by subcategory + collapsible multi-duration groups */}
            {groupedItems ? (
              groupedItems.map(group => (
                <div key={group.id}>
                  <div style={s.subcategoryHeader}>{group.label}</div>
                  {renderGroupedCatalog(group.items)}
                </div>
              ))
            ) : (
              renderGroupedCatalog(filteredItems)
            )}
          </div>
        </div>
