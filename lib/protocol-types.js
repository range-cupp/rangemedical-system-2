...
export const PROTOCOL_TYPES = {
  ...
  hrt_male: {
    ...
    frequencies: [
      { value: 'every_3_5_days', label: 'Every 3.5 days' },
      { value: 'daily', label: 'Daily' },
    ],
    deliveryMethods: [
  ...
  weight_loss_semaglutide: {
    ...
    programTypes: ['weight_loss', 'weight_loss_program', 'weight_loss_semaglutide'],
  ...
  weight_loss_tirzepatide: {
    name: 'Tirzepatide',
    category: 'Weight Loss',
    programTypes: ['weight_loss', 'weight_loss_tirzepatide'],
  ...
  weight_loss_retatrutide: {
    name: 'Retatrutide',
    category: 'Weight Loss',
    programTypes: ['weight_loss', 'weight_loss_retatrutide'],
...
// Get HRT medication name with concentration for a given gender type
// e.g. getHRTMedication('female') → 'Testosterone Cypionate (100mg/ml)'
export function getHRTMedication(hrtType) {
  const key = hrtType === 'female' || hrtType === 'hrt_female' ? 'hrt_female' : 'hrt_male';
  return PROTOCOL_TYPES[key].medications[0];
}

// Get HRT concentration label for display
// e.g. getHRTConcentration('female') → '100mg/ml'
export function getHRTConcentration(hrtType) {
  return hrtType === 'female' || hrtType === 'hrt_female' ? '100mg/ml' : '200mg/ml';
}
...
export function getDeliveryLabel(deliveryMethod, protocolType) {
  ...
  if (deliveryMethod === 'prefilled') return 'Pre-filled';
  // Legacy prefilled_N and prefilled_Xweek values
  if (deliveryMethod.startsWith('prefilled_')) {
    const suffix = deliveryMethod.replace('prefilled_', '');
    if (/^\d+$/.test(suffix)) return `Pre-filled — ${suffix} injection${suffix === '1' ? '' : 's'}`;
    return `Pre-filled (${suffix.replace(/_/g, ' ')})`;
  }
