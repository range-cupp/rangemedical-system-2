...
export async function sendTrialPostSession1({ phone, firstName }) {
  const message = `How are you feeling after your first Red Light session, ${firstName}? You have 2 more sessions this week — we recommend spacing them out (Mon/Wed/Fri works great). Walk in during clinic hours.\n\n- Range Medical (949) 997-3988`;
...
export async function sendTrialExpirationWarning({ phone, firstName, sessionsRemaining }) {
  const message = sessionsRemaining > 0
    ? `Hey ${firstName}, your Red Light Trial expires tomorrow! You still have ${sessionsRemaining === 1 ? '1 session' : `${sessionsRemaining} sessions`} left. Walk in anytime during clinic hours.\n\n- Range Medical (949) 997-3988`
    : `Hey ${firstName}, your Red Light Trial expires tomorrow! If you want to keep going, we have session packs and memberships. Call or text us: (949) 997-3988\n\n- Range Medical`;
