...
import { getAvailableSlots, pickProviderForSlot } from './scheduling';
...
async function executeTool(toolName, toolInput, context) {
  ...
  if (toolName === 'check_availability') {
    ...
    try {
      const result = await getAvailableSlots({ serviceSlug: event_type_slug, date });
      const daySlots = (result?.slots?.[date] || []).map(slot => ({
        start: slot.start,
  ...
  if (toolName === 'book_appointment') {
    ...
    try {
      // Round-robin pick the eligible provider for this slot.
      const provider = await pickProviderForSlot({ serviceSlug: event_type_slug, startISO: start_time });
      if (!provider) {
        return { success: false, error: 'No provider available for that time. Suggest another slot.' };
      }
      ...
      const result = await createAppointment({
        ...
        service_slug: event_type_slug,
        provider: provider.displayLabel || provider.name,
        start_time: startDt.toISOString(),
        end_time: endDt.toISOString(),
        duration_minutes: eventType.duration || 60,
        notes: patient_notes || 'Booked by Jack (patient bot) via SMS',
        visit_reason: `Patient bot: ${eventType.name}`,
        source: 'patient_bot',
        created_by: 'patient_bot',
