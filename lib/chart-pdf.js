...
export async function generateChartPdf({ patient, intakes, activeProtocols, completedProtocols, labs, notes, appointments, serviceLogs, consents, prescriptions }) {
  ...
  const drawSectionHeader = (title) => {
    checkPageBreak(34);
    page.drawRectangle({ x: LEFT_MARGIN, y: yPos - 6, width: CONTENT_WIDTH, height: 24, color: sectionBg });
    page.drawText(title, { x: LEFT_MARGIN + 8, y: yPos, size: 12, font: fontBold, color: white });
    yPos -= 32;
  };
  ...
  const drawLabelValue = (label, value, opts = {}) => {
    ...
    page.drawText(`${label}:`, { x, y: yPos, size: 11, font: fontBold, color: black });
    const labelWidth = fontBold.widthOfTextAtSize(`${label}: `, 11);
    const val = sanitize(String(value || 'N/A')).substring(0, 120);
    page.drawText(val, { x: x + labelWidth, y: yPos, size: 11, font, color: gray });
    yPos -= 18;
  };
  ...
  if (notes && notes.length > 0) {
    for (const note of notes.slice(0, 50)) {
      ...
      // Full note body — no truncation
      const body = (note.body || '');
      // Split by newlines to preserve paragraph structure
      const paragraphs = body.split(/\n+/);
      for (const para of paragraphs) {
        if (para.trim()) {
          drawWrappedText(para.trim(), { size: 11 });
          yPos -= 4;
        }
      }
    }
