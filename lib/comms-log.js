 ...
 * @param {string} [opts.twilioMessageSid]  - Twilio message SID for delivery tracking
 */
export async function logComm({
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message && error.message.includes('twilio_message_sid') && twilioMessageSid) {
