...
// Supports single-item and multi-item receipts
...
export async function generateReceiptPdf({
  ...
  invoiceId,
  date,
  description,        // single-item: string
  items,              // multi-item: [{ name, quantity, unitPriceCents, discountLabel, lineTotalCents }]
  originalAmountCents,
  discountLabel,
  amountPaidCents,
  cardBrand,
  ...
  const clinicAddress = '1901 Westcliff Dr, Suite 10, Newport Beach, CA 92660';
  const clinicAddressWidth = font.widthOfTextAtSize(clinicAddress, 8);
  page.drawText(clinicAddress, {
    x: pageWidth - rightMargin - clinicAddressWidth, y: pageHeight - 40, size: 8, font, color: white,
  });
  ...
  // ===== BILLED TO (name, address & phone) =====
  const hasBilledTo = patientName || patientAddress || patientPhone;
  if (hasBilledTo) {
    page.drawText('BILLED TO', {
      x: leftMargin, y: yPos, size: 8, font: fontBold, color: grayColor,
    });
    yPos -= 16;
    if (patientName) {
    ...
    if (patientAddress) {
      page.drawText(patientAddress, {
        x: leftMargin, y: yPos, size: 9, font, color: grayColor,
      });
      yPos -= 14;
    }
    if (patientPhone) {
      page.drawText(patientPhone, {
        x: leftMargin, y: yPos, size: 9, font, color: grayColor,
      });
      yPos -= 14;
    }
  ...
  // ===== LINE ITEMS (multi-item) =====
  if (isMultiItem) {
    const itemsStartY = yPos;

    page.drawText('ITEMS', {
      x: leftMargin + 14, y: yPos, size: 8, font: fontBold, color: grayColor,
    });
    yPos -= 16;

    for (const item of items) {
      const qty = item.quantity || 1;
      const nameWithQty = qty > 1 ? `${item.name} x${qty}` : item.name;
      const lineTotal = `$${(item.lineTotalCents / 100).toFixed(2)}`;
      const hasItemDiscount = item.unitPriceCents && (item.unitPriceCents * qty) !== item.lineTotalCents;

      // Item name
      page.drawText(nameWithQty, {
        x: leftMargin + 14, y: yPos, size: 9, font, color: black,
      });

      // Line total (right-aligned)
      const lineTotalWidth = font.widthOfTextAtSize(lineTotal, 9);
      page.drawText(lineTotal, {
        x: leftMargin + contentWidth - lineTotalWidth, y: yPos, size: 9, font, color: black,
      });

      // Show original price if discounted
      if (hasItemDiscount) {
        const origPrice = `$${((item.unitPriceCents * qty) / 100).toFixed(2)}`;
        const origWidth = font.widthOfTextAtSize(origPrice, 8);
        page.drawText(origPrice, {
          x: leftMargin + contentWidth - lineTotalWidth - origWidth - 8, y: yPos, size: 8, font, color: grayColor,
        });

        if (item.discountLabel) {
          page.drawText(`(${item.discountLabel})`, {
            x: leftMargin + 14, y: yPos - 12, size: 8, font, color: greenColor,
          });
          yPos -= 12;
        }
      }
    ...
    // Draw the left border line for items
    page.drawRectangle({
      x: leftMargin, y: yPos + 4,
      width: 4, height: itemsStartY - yPos - 4,
      color: black,
    });
  ...
  // ===== AMOUNT BOX =====
  const hasDiscount = !isMultiItem && !isComp && originalAmountCents && originalAmountCents !== amountPaidCents;
