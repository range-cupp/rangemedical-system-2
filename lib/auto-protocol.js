...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}, fulfillmentOpts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'injection': {
      // Derive duration from sessions + frequency (provided by checkout UI).
      // Fall back to sessions × 7 only if frequency missing — legacy behavior.
      const injFrequency = opts.injectionFrequency || null;
      const injDays = calculateProtocolDurationDays(sessions, injFrequency) || sessions * 7;
      const endDate = new Date();
    ...
    case 'peptide': {
      ...
      const method = isInClinicInjections ? 'in_clinic' : (parsedDelivery || opts.deliveryMethod || 'take_home');
...
// ================================================================
async function extendProtocol(protocol, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays, wlConfig, injectionFrequency } = {}) {
...
async function handlePeptideSequentialProtocol({
  ...
  try {
    ...
    // SCENARIO B: Active protocol exists, no queued protocol
      ...
      const queueResult = await createProtocol({
        ...protocolData,
        program_type: 'peptide',
      }, {
        source: 'auto-protocol',
        purchaseId,
        skipDuplicateCheck: true, // queued protocol is intentional
      });

      if (!queueResult.success) throw new Error(`Queued protocol create failed: ${queueResult.error}`);
      const queuedProto = queueResult.protocol;
...
// ================================================================
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId, wlFrequencyDays, wlConfig, injectionFrequency }) {
  try {
    ...
    if (existing) {
      ...
      protocolId = await extendProtocol(existing, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays, wlConfig, injectionFrequency });
      console.log(`Auto-protocol: extended ${protocolType} protocol ${protocolId} for patient ${patientId}`);
    } else {
      // Create new protocol
      const protocolData = buildProtocolData(protocolType, serviceName, patientId, { deliveryMethod, durationDays, medication, patientName, serviceCategory, quantity, wlFrequencyDays, wlConfig, injectionFrequency }, { fulfillmentMethod });
      if (!protocolData) return;

      // Strip _wlConfig before DB insert — it's used for service log creation only
      const { _wlConfig: _wlCfgLocal, ...dbProtocolData } = protocolData;
