...
export async function sendProviderNotification({ type, staff = {}, provider, appointment }) {
  try {
    // Skip notifications for backdated appointments — staff manually logging past
    // visits shouldn't trigger "New booking" alerts to providers (or anyone else).
    if (appointment?.startTime && new Date(appointment.startTime).getTime() < Date.now() - 60 * 1000) {
      console.log(`Skipping provider ${type} notification: appointment start_time is in the past (${appointment.startTime})`);
      return;
    }
    ...
    const result = await sendSMSRouter({
      to: normalized,
      message,
      log: {
        messageType: `provider_${type}`,
        source: 'provider-notifications',
      },
    });
