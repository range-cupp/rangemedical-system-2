...
export async function generateChartPdf({ patient, intakes, activeProtocols, completedProtocols, labs, notes, appointments, serviceLogs, consents, prescriptions, sections = null }) {
  ...
  const formatDate = (d) => {
    if (!d) return 'N/A';
    // YYYY-MM-DD strings from DB date columns have no timezone — parse as local
    // to avoid the UTC-midnight → previous-day-Pacific drift.
  ...
  if (includeSection('visits')) {
    ...
    const KNOWN_CATEGORIES = ['peptide', 'weight_loss', 'testosterone', 'hbot', 'red_light', 'iv_therapy', 'injection'];
    const detectApptCategory = (apt) => {
      const cat = (apt.service_category || '').toLowerCase();
      if (KNOWN_CATEGORIES.includes(cat)) return cat;
    ...
    const dayDelta = (a, b) => {
      // Both args are YYYY-MM-DD; compare in UTC noon to avoid DST edge cases
    ...
    // Greedy match: each service log can dedupe at most one completed appointment.
    // The auto-logger uses todayPacific() at completion time, so the entry_date can
    // be the same day or up to 1 day after the appointment's Pacific date.
    const consumedApptIds = new Set();
    for (const s of (serviceLogs || [])) {
      if (!s.entry_date || !s.category) continue;
      let bestId = null;
      let bestDelta = Infinity;
      for (const a of (appointments || [])) {
        if (!a.id || consumedApptIds.has(a.id)) continue;
        if ((a.status || '').toLowerCase() !== 'completed') continue;
        if (detectApptCategory(a) !== s.category) continue;
        const apptDate = toPacificDate(a.start_time);
        if (!apptDate) continue;
        const delta = Math.abs(dayDelta(s.entry_date, apptDate));
        if (delta <= 1 && delta < bestDelta) {
          bestDelta = delta;
          bestId = a.id;
        }
      }
      if (bestId) consumedApptIds.add(bestId);
    }

    const apptEntries = (appointments || [])
      .filter(a => !consumedApptIds.has(a.id))
      .map(a => ({
        date: a.start_time,
        sortKey: toPacificDate(a.start_time) || '',
        type: 'Appointment',
        detail: a.calendar_name || a.service_name || 'Appointment',
        status: a.status,
      }));

    // Filter phantom service logs: auto-log notes + null medication + another
    // log of same patient/category within ~5 days (signals a duplicate created
    // when staff re-edits an already-completed appointment).
    const otherLogDatesByCategory = new Map();
    for (const s of (serviceLogs || [])) {
      if (!s.entry_date || !s.category) continue;
      if (!s.medication) continue; // only count non-phantom anchors
      if (!otherLogDatesByCategory.has(s.category)) otherLogDatesByCategory.set(s.category, []);
      otherLogDatesByCategory.get(s.category).push(s.entry_date);
    }
    const isPhantom = (s) => {
      if (s.medication) return false;
      const note = (s.notes || '').toLowerCase();
      if (!note.startsWith('auto-logged from completed appointment')) return false;
      const peers = otherLogDatesByCategory.get(s.category) || [];
      return peers.some(d => Math.abs(dayDelta(s.entry_date, d)) <= 5);
    };

    const serviceLogEntries = (serviceLogs || [])
      .filter(s => !isPhantom(s))
      .map(s => ({
        date: s.entry_date,
        sortKey: s.entry_date || '',
        type: s.entry_type || 'Service',
        detail: `${s.category || ''} — ${s.medication || s.notes || ''}`,
        status: 'completed',
      }));

    // Sort by Pacific date string (descending). Stable order keeps appointments
    // before service logs on tie days, which is harmless after dedup.
    const allVisits = [...apptEntries, ...serviceLogEntries]
