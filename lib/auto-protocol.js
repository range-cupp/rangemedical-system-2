...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'hrt': {
      // Parse medication name from service name
      // "Testosterone Cypionate" stays as-is; "HRT Monthly Membership" → "Testosterone Cypionate" default
      let hrtMedication = serviceName;
      const hrtLower = (serviceName || '').toLowerCase();
      if (hrtLower.includes('testosterone')) {
        hrtMedication = 'Testosterone Cypionate';
      } else if (hrtLower.includes('hrt') || hrtLower.includes('membership')) {
        hrtMedication = 'Testosterone Cypionate'; // default HRT medication
      }
