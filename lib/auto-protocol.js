...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'peptide': {
      // Parse medication name from product name: "Peptide Protocol — 10 Day — BPC-157 (500mcg)" → "BPC-157"
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays }) {
  try {
    ...
    if (existing) {
      ...
      // Create new protocol
      const protocolData = buildProtocolData(protocolType, serviceName, patientId, { deliveryMethod, durationDays });
