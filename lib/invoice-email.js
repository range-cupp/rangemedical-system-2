...
export function generateInvoiceHtml({
  ...
  const itemRows = items.map(item => {
    ...
    return `
    <tr>
      <td style="padding: 8px 0; color: #111; font-size: 14px; border-bottom: 1px solid #f0f0f0;">${displayName}${item.quantity > 1 ? ` x${item.quantity}` : ''}</td>
                    ...
                    <tr>
                        <td style="padding: 40px 30px;">
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 0 0 20px;">
                                ...
                                ${shippingCents > 0 ? `
                                <tr>
                                  <td style="padding: 8px 0; color: #111; font-size: 14px; border-bottom: 1px solid #f0f0f0;">Shipping</td>
                                  <td style="padding: 8px 0; color: #111; font-size: 14px; text-align: right; border-bottom: 1px solid #f0f0f0;">$${(shippingCents / 100).toFixed(2)}</td>
                                </tr>` : ''}
                            </table>
                                    ...
                                    <td style="padding: 20px 24px;">
                                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                            ...
                                            <tr>
                                                ...
                                                <td style="padding: 4px 0; color: #666; font-size: 14px; text-align: right;">$${((totalCents + discountCents) / 100).toFixed(2)}</td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 4px 0; color: #16A34A; font-size: 14px;">Discount${discountDescription ? ` (${discountDescription})` : ''}</td>
                                                <td style="padding: 4px 0; color: #16A34A; font-size: 14px; text-align: right;">&minus;$${(discountCents / 100).toFixed(2)}</td>
                                            </tr>
                                            ` : ''}
                                            <tr>
