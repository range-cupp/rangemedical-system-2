...
3. When they give a day/time, use check_availability to verify the slot is open
4. If available, use book_appointment to book it, then confirm with the patient
5. If not available, offer the closest available slots from the results
6. IMPORTANT: Only call book_appointment AFTER the patient has agreed to a specific time
...
10. Do not use emojis excessively — one is fine occasionally.`;
  ...
  try {
    let messages = [
      {
        role: 'user',
        content: `Patient "${patientName}" just texted: "${messageText}"\n\nReply as Jack. Keep it short and move the conversation forward. If they want to come in and you know the service + day, check availability and book it. Always end with a question or next step.`,
      },
    ];
