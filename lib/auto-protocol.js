...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      const injections = qtyMatch ? parseInt(qtyMatch[1]) : 1;
    ...
    case 'peptide': {
      ...
      return {
        ...
        initial_journey_stage: method === 'take_home' ? 'dispensed' : null,
...
async function extendProtocol(protocol, protocolType, serviceName) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      const wlInjections = wlQtyMatch ? parseInt(wlQtyMatch[1]) : 1;
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays }) {
  try {
    ...
    //   "2X Blend: Sermorelin / Ipamorelin" → "Sermorelin / Ipamorelin" (strip blend prefix)
    //   "2X Blend Sermorelin / Ipamorelin Vial" → "Sermorelin / Ipamorelin" (strip blend + vial)
    //   "CJC-1295 / Ipamorelin" → "CJC-1295 / Ipamorelin"
    //   "Tesamorelin / Ipamorelin Vial" → "Tesamorelin / Ipamorelin"
    ...
    if (existing) {
      ...
      if (protocolType === 'weight_loss') {
        ...
        const injCount = wlQty ? parseInt(wlQty) : 1;
        ...
        await supabase.from('service_logs').insert({
          ...
          if (slErr) console.error('Auto service_log error:', slErr);
          else console.log(`Auto-protocol: logged weight_loss pickup for new protocol ${protocolId}`);
        });
      }
    }
