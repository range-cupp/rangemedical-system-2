 ...
 * @param {string} [opts.sentByEmployeeId]  - employee UUID who composed/sent the message
 * @param {string} [opts.sentByEmployeeName] - employee display name
 * @param {string} [opts.htmlBody]           - full rendered HTML of outgoing email
 * @param {string} [opts.mediaUrl]           - image/attachment URL(s) as JSON string
 * @param {Object} [opts.metadata]           - structured context (e.g. { invoice_id })
 */
export async function logComm({
  ...
  try {
    ...
    // Only include sent_by fields if provided (column may not exist yet)
    if (sentByEmployeeId) {
      row.sent_by_employee_id = sentByEmployeeId;
    }
    if (sentByEmployeeName) {
      row.sent_by_employee_name = sentByEmployeeName;
    }

    if (htmlBody) {
      row.html_body = htmlBody;
    }
    ...
    if (error) {
      ...
      if (error.message && (error.message.includes('twilio_message_sid') || error.message.includes('provider') || error.message.includes('sent_by') || error.message.includes('html_body') || error.message.includes('media_url') || error.message.includes('metadata'))) {
        delete row.twilio_message_sid;
        delete row.provider;
        delete row.sent_by_employee_id;
        delete row.sent_by_employee_name;
