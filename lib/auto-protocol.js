  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      if (!wlIsInClinic) {
        ...
        console.log(`Auto-protocol: weight_loss in-clinic injections for ${protocol.patient_id} — sessions will decrement per appointment`);
      }
...
async function handlePeptideSequentialProtocol({
  ...
  // Helper: create a task in the tasks table
  const createTask = async (title, description, priority = 'normal') => {
    ...
    if (error) console.error('Peptide task create error:', error.message);
  };
  ...
  try {
    ...
    if (!activeProtocol && !queuedProtocol) {
      ...
      action = 'CREATED';
      taskTitle = `CREATED \u2014 ${peptideName} for ${patientName || 'patient'} \u00b7 Verify dose and dates`;
      taskBody = `New active protocol created.\nStart: ${today}\nEnd: ${endDate}\nDuration: ${duration} days`;
    ...
    // SCENARIO B: Active protocol exists, no queued protocol
      ...
      action = 'QUEUED';
      taskTitle = `QUEUED \u2014 ${peptideName} for ${patientName || 'patient'} \u00b7 Sequential start: ${newStartDate} \u00b7 Verify no gap`;
      taskBody = `Queued behind active protocol (${activeProtocol.id}).\nActive end: ${activeProtocol.end_date}\nQueued start: ${newStartDate}\nQueued end: ${newEndDate}\nContinuous from: ${continuousDaysStarted}`;

      // 90-day check
      if (is90DayTracked && protocolData.continuous_days_used > 90) {
        const flag = `\n\n\u26a0 90-DAY FLAG: Patient has been on ${peptideName} for ${protocolData.continuous_days_used} days continuously. Provider review recommended.`;
        taskTitle += ` \u26a0 90-DAY`;
        taskBody += flag;
      }
    ...
    // SCENARIO C: Both active AND queued exist
      ...
      action = 'FLAGGED';
      taskPriority = 'high';
      taskTitle = `FLAGGED \u2014 ${peptideName} for ${patientName || 'patient'} \u00b7 Two protocols already exist \u00b7 Damon to resolve manually`;
      taskBody = `Active protocol: ${activeProtocol?.id}\nQueued protocol: ${queuedProtocol?.id}\nNew purchase could not be auto-assigned.\nPurchase: ${serviceName}`;

      // Still check 90-day for flagged
      if (is90DayTracked && activeProtocol) {
        const cds = activeProtocol.continuous_days_started || activeProtocol.start_date;
        if (cds) {
          const qEnd = queuedProtocol?.end_date || activeProtocol.end_date || today;
          const start = new Date(cds + 'T12:00:00');
          const end = new Date(qEnd + 'T12:00:00');
          const totalDays = Math.round((end - start) / (1000 * 60 * 60 * 24));
          if (totalDays > 90) {
            taskBody += `\n\n\u26a0 90-DAY FLAG: Patient has been on ${peptideName} for ${totalDays} days continuously. Provider review recommended.`;
          }
        }
      }
    ...
    // Create the task
    await createTask(taskTitle, taskBody, taskPriority);
    ...
    // SYSTEM ERROR — create task for Chris
    await supabase.from('tasks').insert({
      patient_id: patientId || null,
      patient_name: patientName || null,
      category: 'peptide',
      title: `SYSTEM ERROR \u2014 Peptide protocol failed for ${patientName || 'unknown patient'}`,
      description: `Error: ${err.message}\nService: ${serviceName}\nPurchase ID: ${purchaseId || 'unknown'}`,
      status: 'open',
      priority: 'high',
      due_date: today,
    }).catch(taskErr => console.error('System error task creation also failed:', taskErr.message));
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId }) {
  try {
    ...
    // ── WEIGHT LOSS: Staff-managed only ──
    // Purchases get linked to existing protocols (for billing correlation),
    // but NO auto-creation, NO auto-extension. Task Evan + Lily to manage.
    if (protocolType === 'weight_loss') {
      const existing = await findExistingProtocol(patientId, protocolType, medication);
      const protocolId = existing?.id || null;

      if (protocolId && purchaseId) {
        await supabase
          .from('purchases')
          .update({ protocol_id: protocolId, protocol_created: false })
          .eq('id', purchaseId);
      }

      // Create tasks for Evan and Lily
      const EVAN_ID = 'd1368f6d-f0b2-4ad7-a476-26561571f103';
      const LILY_ID = '29529f97-0a76-48f4-a9f7-38fd608a1cae';
      const todayStr = new Date().toISOString().split('T')[0];
      const taskTitle = existing
        ? `Weight Loss: Extend protocol for ${patientName || 'patient'} — ${medication || serviceName}`
        : `Weight Loss: Create protocol for ${patientName || 'patient'} — ${medication || serviceName}`;
      const taskDesc = `Purchase: ${serviceName}\nMedication: ${medication || 'See purchase'}\n\n${existing ? 'Extend the existing protocol.' : 'Create a new weight loss protocol in the service log.'}`;

      for (const assignee of [EVAN_ID, LILY_ID]) {
        await supabase.from('tasks').insert({
          title: taskTitle,
          ...
          if (tErr) console.error('Auto-protocol: WL task create error:', tErr);
        });
      }

      console.log(`Auto-protocol: weight_loss purchase — linked to ${protocolId || 'none'}, tasked Evan + Lily`);
      return;
    }
    ...
    if (existing) {
      // Non-weight-loss: extend existing protocol as before
      ...
      // Auto-log pickup for new weight loss protocols (skip for in-clinic injections — those log per appointment)
      if (protocolType === 'weight_loss' && fulfillmentMethod !== 'in_clinic_injections') {
