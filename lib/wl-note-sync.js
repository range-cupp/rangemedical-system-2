 ...
 * @param {object} options - { approvedDoseChangeRequestId }
...
export async function syncWLNoteToServiceLog(supabase, note, options = {}) {
  ...
  const { data: protocols } = await supabase
    .from('protocols')
    .select('id, starting_weight, sessions_used, total_sessions, selected_dose, dose, dose_per_injection, injections_per_week, patient_id, medication, program_type, category, goal_weight')
  ...
  if (hasWeight) {
    ...
    try {
      ...
      if (note.appointment_id) {
        vitalsData.appointment_id = note.appointment_id;
        const { data: existingVitals } = await supabase
          ...
          .maybeSingle();
        if (existingVitals) {
          await supabase.from('patient_vitals').update(vitalsData).eq('id', existingVitals.id);
        } else {
          await supabase.from('patient_vitals').insert(vitalsData);
        }
      } else {
        await supabase.from('patient_vitals').insert(vitalsData);
      }
