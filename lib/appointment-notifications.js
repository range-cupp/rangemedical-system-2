...
function buildBloodDrawSMS(firstName, date, time, gender, type) {
  const base = type === 'reschedule'
    ? `Hi ${firstName}, your blood draw has been rescheduled to ${date} at ${time}.`
    : `Hi ${firstName}! Your blood draw is confirmed for ${date} at ${time}.`;

  let prep = ' Please fast 10-12 hrs before (water & black coffee ok), stay hydrated, and avoid NSAIDs for 48 hrs.';

  if (gender === 'male') {
    prep += ' Hold testosterone injections for 3 days before. Avoid heavy workouts & sexual activity 24 hrs before (PSA accuracy).';
  } else if (gender === 'female') {
    prep += ' If cycling, Day 3 of your period is best for accurate hormone results — call (949) 997-3988 if timing doesn\'t line up. Continue estrogen & progesterone as normal; hold testosterone injections 3 days.';
  }

  return `${base}${prep} Full prep guide: https://www.range-medical.com/lab-prep — See you at Range Medical!`;
}

const SMS_TEMPLATES = {
  confirmation: ({ firstName, serviceName, date, time, isBloodDraw, gender }) =>
    isBloodDraw
      ? buildBloodDrawSMS(firstName, date, time, gender, 'confirmation')
  ...
  reschedule: ({ firstName, serviceName, date, time, isBloodDraw, gender }) =>
    isBloodDraw
      ? buildBloodDrawSMS(firstName, date, time, gender, 'reschedule')
