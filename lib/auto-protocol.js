    ...
    case 'peptide': {
      ...
      if (catalogMatch) {
        ...
        if (catalogMatch.phases && parsedPhase) {
          ...
          dose = phase1?.dose || phase1?.doses?.[duration] || catalogMatch.dose;
          frequency = phase1?.frequencies?.[duration] || catalogMatch.defaultFrequency;
          totalInjections = phase1?.injections?.[duration] || catalogMatch.totalInjections || duration;
        } else {
          dose = catalogMatch.dose;
          frequency = catalogMatch.defaultFrequency;
          totalInjections = catalogMatch.totalInjections || duration;
        }
...
async function extendProtocol(protocol, protocolType, serviceName) {
  ...
  switch (protocolType) {
    ...
    case 'hrt': {
      ...
      // Auto-log pickup for HRT take-home extensions
      if (protocol.delivery_method === 'take_home') {
        ...
        else console.log(`Auto-protocol: logged HRT pickup for extended protocol ${protocol.id}`);
      }
...
// ================================================================
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity }) {
  try {
    ...
    if (existing) {
      ...
      // Auto-log pickup for new HRT take-home protocols
      if (protocolType === 'hrt' && protocolData.delivery_method === 'take_home') {
        const todayStr = new Date().toISOString().split('T')[0];
        ...
        if (hrtSlErr) console.error('Auto service_log error:', hrtSlErr);
        else console.log(`Auto-protocol: logged HRT pickup for new protocol ${protocolId}`);
      }
    }
