...
export function getProtocolTracking(protocol) {
  ...
  if (isWeightLoss) {
    ...
    if (protocol.next_expected_date) {
      ...
      // In-clinic patients don't have refills — they pay per block and come in weekly.
      // Only show session-based tracking, not refill urgency.
      const statusText = delivery === 'in_clinic'
        ? `${sessionsUsed}/${totalSessions} injections`
        : daysUntil <= 0 ? 'Refill overdue'
        : daysUntil <= 3 ? `${daysUntil}d — Refill soon`
        : `${daysUntil} days until refill`;

      const urgency = delivery === 'in_clinic'
        ? (sessionsUsed >= totalSessions ? 'completed' : 'active')
        : daysUntil <= 0 ? 'overdue' : daysUntil <= 3 ? 'ending_soon' : daysUntil <= 14 ? 'active' : 'just_started';

      return {
        days_remaining: delivery === 'in_clinic' ? null : daysUntil,
        sessions_used: sessionsUsed,
        total_sessions: totalSessions,
        status_text: statusText,
        tracking_type: delivery === 'in_clinic' ? 'session_based' : 'time_based',
        urgency,
    ...
    // In-clinic patients: skip refill tracking entirely, use session-based display
    if (delivery === 'in_clinic') {
      return {
        sessions_used: sessionsUsed,
        total_sessions: totalSessions,
...
export function getWLStatus(protocol) {
  ...
  const deliveryMethod = (protocol.delivery_method || '').toLowerCase();
  const isInClinic = deliveryMethod === 'in_clinic';

  // In-clinic patients: track by sessions only, not refill dates
  if (isInClinic) {
    if (sessionsExhausted) return 'overdue'; // needs to pay for next block
    if (totalSessions > 0 && (totalSessions - sessionsUsed) <= 2) return 'due_soon';
    return 'on_track';
  }
