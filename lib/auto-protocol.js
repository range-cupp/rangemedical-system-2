...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber }) {
  try {
    ...
    // Purchases still get linked to existing protocols (for billing correlation),
    // but NO auto-creation, NO auto-extension, NO auto service log entries.
    // Medical staff (Lily, Burgess, Evan) manage protocols + log encounters manually.
    if (protocolType === 'weight_loss') {
      ...
      console.log(`Auto-protocol: weight_loss purchase for patient ${patientId} — linked to protocol ${protocolId || 'none'} (staff will manage protocol + encounters)`);
      return;
    }
