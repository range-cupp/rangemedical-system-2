...
import { getEventTypes, getAvailableSlots, createBooking, reassignBooking } from './calcom';
...
async function handleBookAppointment(params) {
  try {
    ...
    if (providerInfo?.calcom_user_id) {
      const reassigned = await reassignBooking(booking.uid, providerInfo.calcom_user_id)
        .catch(() => null);
