// pages/review/[token].jsx
// Review gift landing page — patient picks injection type, selects a time, books
...
export default function ReviewGift() {
  ...
  useEffect(() => {
    ...
    async function validate() {
      try {
        const res = await fetch(`/api/review/validate?token=${token}`);
  ...
  useEffect(() => {
    ...
    async function fetchSlots() {
      try {
        const res = await fetch(`/api/review/slots?type=${selectedType}&date=${selectedDate}&eventTypeId=${eventTypeId}`);
        const data = await res.json();
        if (data.slots) {
          const allSlots = [];
  ...
  if (loading) {
    return (
      <Layout title="Your Gift | Range Medical">
  ...
  return (
    <Layout title="Thank You! | Range Medical" description="Your free injection from Range Medical.">
      <Head><meta name="robots" content="noindex, nofollow" /></Head>
      ...
      <section style={s.hero}>
        <div style={s.container}>
          ...
          <h1 style={s.heroTitle}>Thanks, {firstName}!</h1>
          <p style={s.heroSub}>
            We really appreciate you taking the time to leave us a review. As a thank you, enjoy a <strong>free injection</strong> on us.
          </p>
        </div>
      </section>
          ...
          <div style={s.container}>
            <div style={s.confirmedCard}>
              ...
              <p style={s.confirmedText}>
                Your free injection is booked. You will receive a confirmation text shortly with the details.
              </p>
              <p style={s.confirmedText}>
                Thanks again for the review. We will see you soon!
              </p>
      ...
      <div style={{ textAlign: 'center', padding: '24px', color: '#9ca3af', fontSize: '13px' }}>
        Gift valid for 30 days. One free injection per patient.
      </div>
    </Layout>
  );
}
