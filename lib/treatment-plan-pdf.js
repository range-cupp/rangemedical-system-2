...
export async function generateTreatmentPlanPdf({
  ...
  // Next Steps — patient-specific action items, or generic fallback
  drawSectionHeader('Next Steps');
  const cleanNextSteps = (nextSteps || []).filter(b => b && b.trim());
  if (cleanNextSteps.length > 0) {
    for (const step of cleanNextSteps) {
      drawBullet(step);
    }
    yPos -= 6;
    drawWrappedLine(
      'Questions about any of this? Call or text us at (949) 997-3988.',
