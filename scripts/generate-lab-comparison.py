...
title_s     = st('Title', fontName='Helvetica-Bold', fontSize=18, textColor=BLACK, leading=21, spaceAfter=2)
intro_s     = st('Intro', fontName='Helvetica', fontSize=9.5, textColor=MID_GRAY, leading=13, spaceAfter=4)
sec_s       = st('Sec', fontName='Helvetica-Bold', fontSize=9, textColor=MID_GRAY, leading=12, spaceBefore=0, spaceAfter=2)
opt_title_s = st('OptT', fontName='Helvetica-Bold', fontSize=12, textColor=BLACK, leading=15)
opt_sub_s   = st('OptSub', fontName='Helvetica-Oblique', fontSize=9, textColor=MID_GRAY, leading=12)
price_s     = st('Price', fontName='Helvetica-Bold', fontSize=12, textColor=ACCENT, leading=15)
bestif_s    = st('Best', fontName='Helvetica', fontSize=9, textColor=DARK_GRAY, leading=12, leftIndent=10, firstLineIndent=-10, spaceAfter=1)
grp_hdr_s   = st('GrpH', fontName='Helvetica-Bold', fontSize=8.5, textColor=MID_GRAY, leading=11, spaceBefore=4, spaceAfter=1)
bio_s       = st('Bio', fontName='Helvetica', fontSize=8.5, textColor=DARK_GRAY, leading=11, leftIndent=8, spaceAfter=0)
gender_s    = st('Gen', fontName='Helvetica-Bold', fontSize=8, textColor=ACCENT, leading=10.5, spaceBefore=3, spaceAfter=0)
gender_b_s  = st('GenB', fontName='Helvetica', fontSize=8.5, textColor=DARK_GRAY, leading=11, leftIndent=8, spaceAfter=0)
addon_hdr_s = st('AddH', fontName='Helvetica-Bold', fontSize=9, textColor=MID_GRAY, leading=12)
addon_s     = st('Add', fontName='Helvetica', fontSize=9, textColor=DARK_GRAY, leading=12)
step_s      = st('Step', fontName='Helvetica', fontSize=9, textColor=DARK_GRAY, leading=12, leftIndent=14, firstLineIndent=-14, spaceAfter=1)
foot_s      = st('Foot', fontName='Helvetica-Oblique', fontSize=7.5, textColor=MID_GRAY, leading=10)
foot_b_s    = st('FootB', fontName='Helvetica-Bold', fontSize=8, textColor=DARK_GRAY, leading=11)
chk_s       = st('Chk', fontName='Helvetica-Bold', fontSize=9, textColor=GREEN, leading=12, leftIndent=10, firstLineIndent=-10, spaceAfter=1)
...
def build_essential_content():
    ...
    for b in [
        f'Want a strong {LQ}baseline{RQ} of your health',
...
def build_elite_content():
    ...
    items.append(Spacer(1, 3))
    items.append(Paragraph(f'<b>Includes everything in Essential, plus (78 biomarkers total):</b>', grp_hdr_s))

    groups = [
        (f'Heart &amp; inflammation {LQ}early warning{RQ} markers',
