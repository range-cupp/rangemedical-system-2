// /api/treatment-plan-v2/generate
// Structured treatment plan builder — preview, save, or send.
// Accepts structured data (symptoms, goals, plan categories, lifestyle, follow-up).

import { createClient } from '@supabase/supabase-js';
import { Resend } from 'resend';
import { generateTreatmentPlanV2Pdf } from '../../../lib/treatment-plan-v2-pdf';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff; border-radius:12px; overflow:hidden; box-shadow:0 4px 6px rgba(0,0,0,0.05);">
        ...
        <tr><td style="padding:16px 40px 24px;">
          <p style="margin:0; font-size:16px; color:#525252; line-height:1.7;">
            Attached is your personalized treatment plan from your Range Medical provider.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pdfBytes = await generateTreatmentPlanV2Pdf({
      patientName,
      planDate,
    ...
    if (mode === 'save' || mode === 'send') {
      if (mode === 'send') {
        if (!patient.email) return res.status(400).json({ error: 'Patient has no email on file' });
      ...
      if (uploadErr) {
        console.error('Treatment plan v2 upload error:', uploadErr);
      ...
      await supabase.from('medical_documents').insert({
        patient_id,
        document_name: `Treatment Plan - ${dateStr}`,
        document_url: urlData?.signedUrl || null,
        document_type: 'Treatment Plan',
        file_path: filePath,
        file_size: buffer.length,
        notes: `Structured treatment plan (v2)`,
      ...
      if (mode === 'send') {
        ...
        if (emailErr) {
          console.error('Treatment plan v2 email error:', emailErr);
    ...
    console.error('Treatment plan v2 generate error:', err);
