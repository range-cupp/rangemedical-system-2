...
import { sendEmailCopy } from './sms-email-copy';
 ...
 */
export async function sendSMS({ to, message, provider: providerOverride, mediaUrl, log, skipOptOutCheck, skipEmailCopy, patientEmail, patientName }) {
  // Check patient opt-out preferences before sending (when patientId is available)
  if (!skipOptOutCheck && log?.patientId) {
    const isAutomation = !!(log.source && log.source !== 'manual');
    ...
    if (!optOut.allowed) {
      console.log(`SMS blocked for patient ${log.patientId}: ${optOut.reason}`);
      if (log) {
        try {
          await logComm({
            channel: 'sms',
            ...
            recipient: to,
            status: 'blocked',
            errorMessage: optOut.reason,
            direction: 'outbound',
          });
        } catch (logErr) {
          console.error('sendSMS opt-out log error:', logErr.message);
        }
      }
      return { success: false, error: optOut.reason, blocked: true };
    }
  }
  ...
  // Send email copy of every outbound SMS (non-blocking)
  if (!skipEmailCopy) {
    sendEmailCopy({
      to,
      message,
      ...
      protocolId: log?.protocolId || null,
    }).catch(err => console.error('SMS email copy failed:', err.message));
  }
