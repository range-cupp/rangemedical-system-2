...
export const ENCOUNTER_TEMPLATES = {
  iv_therapy: {
    label: 'Range IV (Full Vitamin/Mineral)',
  ...
  // ── Specialty IV sub-types (share iv_therapy form + templates) ───────────
  nad_iv: {
    label: 'NAD+ IV',
    parentType: 'iv_therapy',
    defaultNoteType: 'procedure',
    quickNotes: [
      'Tolerated well',
      'No adverse reactions',
  ...
  supplement: {
    ...
    quickNotes: [
      'Product dispensed',
      'Usage instructions reviewed',
      'Patient verbalized understanding',
    ],
  },
  blood_draw_encounter: {
    label: 'Blood Draw / Phlebotomy',
    parentType: 'injection',
    defaultNoteType: 'procedure',
    quickNotes: [
      'Antecubital fossa',
      'Butterfly needle',
      'Patient tolerated well',
      'Gauze and Coban applied',
      'No hematoma',
    ],
  },
};
...
export function getTemplatesForCategory(templateKey) {
  // Resolve parentType for specialty sub-types (e.g., nad_iv → iv_therapy)
  const effectiveKey = ENCOUNTER_TEMPLATES[templateKey]?.parentType || templateKey;
  const categoryMap = {
    ...
    general: null,
    supplement: null,
    blood_draw_encounter: 'injection',
  };
  const category = categoryMap[effectiveKey];
  const matched = [];
  const other = [];
