...
// Range Medical - Updated 2026-01-28
// Added 1-week prefilled option, Labs Pipeline Tab
...
export default function UnifiedPipeline() {
  ...
  const fetchPatients = async () => {
    try {
      ...
      const json = await res.json();
      if (json.patients) setPatients(json.patients);
    } catch (err) {
  ...
  const getFilteredProtocols = () => {
    ...
    if (statusFilter === 'active' || statusFilter === 'all') {
      protocols = [...(data.protocols.ending_soon || []), ...(data.protocols.active || []), ...(data.protocols.just_started || [])];
    }
    if (statusFilter === 'completed' || statusFilter === 'all') {
      protocols = [...protocols, ...(data.protocols.completed || [])];
    }
    if (deliveryFilter !== 'all') protocols = protocols.filter(p => p.delivery === deliveryFilter);
    if (categoryFilter !== 'all') protocols = protocols.filter(p => p.category === categoryFilter);
    if (searchTerm) {
      const term = searchTerm.toLowerCase();
      protocols = protocols.filter(p => (p.patient_name || '').toLowerCase().includes(term) || (p.medication || '').toLowerCase().includes(term) || (p.program_name || '').toLowerCase().includes(term));
    }
    return protocols;
  };
  ...
  const formatDate = (dateStr) => {
    if (!dateStr) return '-';
    let d = dateStr.length === 10 ? new Date(dateStr + 'T00:00:00') : new Date(dateStr);
  ...
  const openGHL = (ghlId) => {
    if (!ghlId) { showToast('No GHL contact linked', 'error'); return; }
  ...
  const confirmRenew = async () => {
    ...
    try {
      const res = await fetch(`/api/protocols/${renewModal.protocol.id}/renew`, {
        ...
        body: JSON.stringify({ duration_days: parseInt(renewModal.duration), start_date: renewModal.startDate })
      });
      const result = await res.json();
      if (result.success) { showToast('Protocol extended!'); closeRenewModal(); fetchData(); }
      else showToast(result.error || 'Failed to renew', 'error');
    } catch (err) { showToast('Error extending protocol', 'error'); }
    finally { setRenewingId(null); }
  };

  const deleteProtocol = async (protocol) => {
    if (!confirm(`DELETE protocol for ${protocol.patient_name}?\n\n${protocol.medication || protocol.program_name}\n\nThis cannot be undone.`)) return;
    try {
      const res = await fetch(`/api/protocols/${protocol.id}`, { method: 'DELETE' });
      const result = await res.json();
      if (result.success) { showToast('Protocol deleted'); fetchData(); }
      else showToast(result.error || 'Failed to delete', 'error');
    } catch (err) { showToast('Error deleting protocol', 'error'); }
  };
  ...
  const submitStartProtocol = async () => {
    ...
    if (!protocolType) { showToast('Please select a protocol type', 'error'); return; }
    setSubmitting(true);
    const payload = {
    ...
    switch (protocolType) {
      ...
      case 'iv':
        ...
        payload.total_sessions = protocolForm.iv_package === 'Single' ? 1 : protocolForm.iv_package === '5 Pack' ? 5 : 10;
        payload.delivery_method = 'in_clinic';
        break;
      case 'hbot':
        payload.program_name = protocolForm.hbot_package;
        payload.total_sessions = protocolForm.hbot_package === 'Single' ? 1 : protocolForm.hbot_package === '5 Pack' ? 5 : 10;
        payload.delivery_method = 'in_clinic';
        break;
      case 'rlt':
        payload.program_name = protocolForm.rlt_package;
        payload.total_sessions = protocolForm.rlt_package === 'Single' ? 1 : protocolForm.rlt_package === '5 Pack' ? 5 : 10;
        payload.delivery_method = 'in_clinic';
        break;
      case 'injection':
        payload.program_name = protocolForm.injection_package;
        payload.medication = protocolForm.injection_type;
        payload.total_sessions = protocolForm.injection_package === 'Single' ? 1 : protocolForm.injection_package === '5 Pack' ? 5 : 10;
        payload.delivery_method = protocolForm.injection_delivery;
        break;
    }
    try {
      const res = await fetch('/api/protocols/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const result = await res.json();
      if (result.success) {
        ...
        if (protocolType === 'hrt' || protocolType === 'weight_loss') {
          try {
            await fetch('/api/protocols/schedule-follow-up-labs', {
              ...
              body: JSON.stringify({ protocolId: result.protocol?.id, patientId: selectedPatient.id, patientName, ghlContactId: selectedPatient.ghl_contact_id, programType: protocolType, startDate: protocolForm.start_date })
            });
          } catch (err) { console.log('Follow-up scheduling skipped:', err); }
        }
      } else { showToast(result.error || 'Failed to create protocol', 'error'); }
    } catch (err) { showToast('Error creating protocol', 'error'); }
  ...
  const submitLog = async () => {
    ...
    if (category === 'weight_loss') payload.weight = logForm.weight;
    ...
    try {
      ...
      const result = await res.json();
      if (result.success) { showToast('Activity logged!'); closeLogModal(); fetchData(); }
      else showToast(result.error || 'Failed to log activity', 'error');
    } catch (err) { showToast('Error logging activity', 'error'); }
    finally { setSubmitting(false); }
  };
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/protocols/${editModal.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const result = await res.json();
      if (result.success) { showToast('Protocol updated!'); closeEditModal(); fetchData(); }
      else showToast(result.error || 'Failed to update', 'error');
    } catch (err) { showToast('Error updating protocol', 'error'); }
    finally { setSubmitting(false); }
  };
    ...
    return (
      <tr key={protocol.id} style={styles.row}>
        ...
        <td style={styles.cell}>
          ...
          {protocol.category === 'weight_loss' && (protocol.starting_weight || protocol.current_weight) && (
            <div style={{ fontSize: '11px', marginTop: '4px', color: '#6b7280' }}>
              ...
              {protocol.weight_lost > 0 && <span style={{ color: '#059669', fontWeight: '600', marginLeft: '6px' }}>↓{protocol.weight_lost} lbs</span>}
              {protocol.weight_lost < 0 && <span style={{ color: '#dc2626', fontWeight: '600', marginLeft: '6px' }}>↑{Math.abs(protocol.weight_lost)} lbs</span>}
            </div>
          )}
        </td>
        ...
        <td style={styles.cell}>
          ...
          <div style={{ fontSize: '11px', color: '#9ca3af' }}>{protocol.last_refill_date ? 'Last refill' : 'Started'}</div>
        </td>
        <td style={styles.cell}>
          <span style={{ ...styles.daysLeft, color: isOverdue ? '#dc2626' : isEndingSoon ? '#f59e0b' : '#059669', background: isOverdue ? '#fef2f2' : isEndingSoon ? '#fffbeb' : '#f0fdf4' }}>{protocol.status_text}</span>
        </td>
        <td style={styles.cell}>
          <span style={{ ...styles.deliveryBadge, background: protocol.delivery === 'take_home' ? '#dbeafe' : '#f3f4f6', color: protocol.delivery === 'take_home' ? '#1d4ed8' : '#374151' }}>{protocol.delivery === 'take_home' ? 'TAKE HOME' : 'IN CLINIC'}</span>
        </td>
        <td style={styles.cellActions}>
          <button style={{ ...styles.actionBtn, ...styles.editBtn }} onClick={() => openEditModal(protocol)} title="Edit">✏️ Edit</button>
          <button style={{ ...styles.actionBtn, ...styles.logBtn }} onClick={() => openLogModal(protocol)} title="Log">➕ Log</button>
          <button style={styles.actionBtn} onClick={() => openGHL(protocol.ghl_contact_id)} title="SMS">📱 SMS</button>
          <button style={styles.actionBtn} onClick={() => openGHL(protocol.ghl_contact_id)} title="GHL">↗ GHL</button>
          {protocol.category === 'peptide' && (isEndingSoon || isOverdue || protocol.status === 'completed') && (
            <button style={{ ...styles.actionBtn, ...styles.renewBtn }} onClick={() => openRenewModal(protocol)} title="Extend">⏳</button>
          )}
          <button style={{ ...styles.actionBtn, ...styles.deleteBtn }} onClick={() => deleteProtocol(protocol)} title="Delete">🗑️</button>
        </td>
      </tr>
    );
  };
  ...
  const renderTable = () => {
    ...
    if (protocols.length === 0) return <div style={styles.emptyState}><div style={{ fontSize: '48px', marginBottom: '12px' }}>📋</div><p>No protocols found</p></div>;
    return (
      <div style={styles.tableContainer}>
        <table style={styles.table}>
          ...
          <tbody>{protocols.map(p => renderRow(p))}</tbody>
        </table>
      </div>
    );
  };
        ...
        return (
          <>
            <div style={styles.formGroup}>
              ...
              <select value={protocolForm.wl_medication || ''} onChange={(e) => setProtocolForm({ ...protocolForm, wl_medication: e.target.value, wl_starting_dose: '' })} style={styles.formSelect} required>
            ...
            <div style={styles.formGroup}>
              ...
              <select value={protocolForm.wl_starting_dose || ''} onChange={(e) => setProtocolForm({ ...protocolForm, wl_starting_dose: e.target.value })} style={styles.formSelect}>
                ...
                {protocolForm.wl_medication === 'Semaglutide' ? ['0.25mg', '0.5mg', '1mg', '1.7mg', '2.4mg'].map(d => <option key={d} value={d}>{d}</option>) : ['1mg', '2mg', '3mg', '4mg', '5mg', '6mg', '7mg', '8mg', '9mg', '10mg', '11mg', '12mg', '13mg', '14mg', '15mg'].map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Delivery *</label>
              <select value={protocolForm.wl_delivery || ''} onChange={(e) => setProtocolForm({ ...protocolForm, wl_delivery: e.target.value })} style={styles.formSelect} required>
            ...
            <div style={styles.formGroup}>
              ...
              <select value={protocolForm.wl_total_injections || '4'} onChange={(e) => setProtocolForm({ ...protocolForm, wl_total_injections: e.target.value })} style={styles.formSelect}>
        ...
        return (
          <>
            ...
            <div style={styles.formGroup}>
              ...
              ) : <input type="text" value={protocolForm.peptide_dosage || ''} onChange={(e) => setProtocolForm({ ...protocolForm, peptide_dosage: e.target.value })} placeholder="Select a peptide first..." style={styles.formInput} />}
            </div>
            {protocolForm.peptide_dosage === 'custom' && (
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Custom Dose</label>
                <input type="text" value={protocolForm.peptide_custom_dose || ''} onChange={(e) => setProtocolForm({ ...protocolForm, peptide_custom_dose: e.target.value, peptide_dosage: e.target.value })} placeholder="e.g. 400mcg" style={styles.formInput} />
              </div>
            )}
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Frequency</label>
              <select value={protocolForm.peptide_frequency || 'Daily'} onChange={(e) => setProtocolForm({ ...protocolForm, peptide_frequency: e.target.value })} style={styles.formSelect}>
                <option value="">Select frequency...</option>
                {selectedPeptideInfo?.frequency && <option value={selectedPeptideInfo.frequency}>{selectedPeptideInfo.frequency} (Recommended)</option>}
            ...
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Delivery *</label>
              <select value={protocolForm.peptide_delivery || ''} onChange={(e) => setProtocolForm({ ...protocolForm, peptide_delivery: e.target.value })} style={styles.formSelect} required>
                <option value="">Select delivery...</option>
        ...
        return (
          <>
            <div style={styles.formGroup}>
              ...
              <select value={protocolForm.hrt_type || 'male'} onChange={(e) => setProtocolForm({ ...protocolForm, hrt_type: e.target.value })} style={styles.formSelect}>
            ...
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Medication</label>
              <select value={protocolForm.hrt_medication || ''} onChange={(e) => setProtocolForm({ ...protocolForm, hrt_medication: e.target.value })} style={styles.formSelect}>
                <option value="">Select medication...</option>
        ...
        return (
          <>
            <div style={styles.formGroup}>
              ...
              <select value={protocolForm.iv_package || ''} onChange={(e) => setProtocolForm({ ...protocolForm, iv_package: e.target.value })} style={styles.formSelect} required>
      ...
      case 'hbot':
        return (
          <>
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Package *</label>
              <select value={protocolForm.hbot_package || ''} onChange={(e) => setProtocolForm({ ...protocolForm, hbot_package: e.target.value })} style={styles.formSelect} required>
                <option value="">Select package...</option>
                <option value="Single">Single Session</option>
                <option value="5 Pack">5 Pack</option>
                <option value="10 Pack">10 Pack</option>
      ...
      case 'rlt':
        return (
          <>
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Package *</label>
              <select value={protocolForm.rlt_package || ''} onChange={(e) => setProtocolForm({ ...protocolForm, rlt_package: e.target.value })} style={styles.formSelect} required>
                <option value="">Select package...</option>
                <option value="Single">Single Session</option>
                <option value="5 Pack">5 Pack</option>
                <option value="10 Pack">10 Pack</option>
      ...
      case 'injection':
        return (
          <>
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Package *</label>
              <select value={protocolForm.injection_package || ''} onChange={(e) => setProtocolForm({ ...protocolForm, injection_package: e.target.value })} style={styles.formSelect} required>
                <option value="">Select package...</option>
                <option value="Single">Single Session</option>
                <option value="5 Pack">5 Pack</option>
                <option value="10 Pack">10 Pack</option>
            ...
            <div style={styles.formGroup}>
              <label style={styles.formLabel}>Delivery *</label>
              <select value={protocolForm.injection_delivery || ''} onChange={(e) => setProtocolForm({ ...protocolForm, injection_delivery: e.target.value })} style={styles.formSelect} required>
                <option value="">Select delivery...</option>
                <option value="in_clinic">In Clinic</option>
                <option value="take_home">Take Home</option>
              </select>
  ...
  const renderLogFields = () => {
      ...
      return (
        <>
          <div style={styles.formGroup}>
            ...
            <input type="number" step="0.1" value={logForm.weight || ''} onChange={(e) => setLogForm({ ...logForm, weight: e.target.value })} placeholder="e.g. 215.5" style={styles.formInput} required />
          </div>
          <div style={styles.formGroup}>
            ...
            <select value={logForm.dosage || logModal.dose || ''} onChange={(e) => setLogForm({ ...logForm, dosage: e.target.value })} style={styles.formSelect}>
              ...
              {(logModal.medication || '').toLowerCase().includes('semaglutide') ? ['0.25mg', '0.5mg', '1mg', '1.7mg', '2.4mg'].map(d => <option key={d} value={d}>{d}</option>) : ['1mg', '2mg', '3mg', '4mg', '5mg', '6mg', '7mg', '8mg', '9mg', '10mg', '11mg', '12mg', '13mg', '14mg', '15mg'].map(d => <option key={d} value={d}>{d}</option>)}
            </select>
          </div>
        </>
      );
        ...
        <>
          ...
          {logForm.log_type === 'injection' && (
            <>
              <div style={styles.formGroup}>
                ...
                <input type="text" value={logForm.dosage || logModal.dose || ''} onChange={(e) => setLogForm({ ...logForm, dosage: e.target.value })} placeholder={logModal.dose || 'e.g. 100mg'} style={styles.formInput} />
              </div>
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Injection Site</label>
                <select value={logForm.site || ''} onChange={(e) => setLogForm({ ...logForm, site: e.target.value })} style={styles.formSelect}>
          ...
          {logForm.log_type === 'pickup' && (
            <>
              ...
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Dose</label>
                <input type="text" value={logForm.dosage || logModal.dose || ''} onChange={(e) => setLogForm({ ...logForm, dosage: e.target.value })} placeholder={logModal.dose || 'e.g. 0.4ml/80mg'} style={styles.formInput} />
              </div>
            </>
          )}
          {logForm.log_type === 'blood_draw' && (
            <div style={styles.formGroup}>
              ...
              <select value={logForm.panel_type || ''} onChange={(e) => setLogForm({ ...logForm, panel_type: e.target.value })} style={styles.formSelect} required>
    ...
    return <div style={styles.infoBox}>✓ Logging session for {logModal.medication || logModal.program_name}</div>;
  };
          ...
          <>
            <div style={styles.header}>
              ...
              <div style={styles.headerActions}>
                <input type="text" placeholder="Search protocols..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={styles.searchInput} />
                <div style={styles.tabs}>
                  {['active', 'completed'].map(status => <button key={status} style={{ ...styles.tab, ...(statusFilter === status ? styles.tabActive : {}) }} onClick={() => setStatusFilter(status)}>{status.charAt(0).toUpperCase() + status.slice(1)}</button>)}
                </div>
                <div style={styles.tabs}>
            ...
            {data && data.counts && (
              <div style={styles.statsBar}>
                <div style={{ ...styles.statCard, borderLeftColor: '#dc2626' }}><div style={styles.statLabel}>ENDING SOON (≤3 DAYS)</div><div style={styles.statValue}>{data.counts.ending_soon || 0}</div></div>
            ...
            {data?.purchases?.needs_protocol?.length > 0 && (
              <div style={styles.purchasesSection}>
                <div style={styles.purchasesHeader}>
                  <h3 style={styles.purchasesTitle}>💳 Payments Needing Protocol ({data.purchases.needs_protocol.length})</h3>
                  <span style={styles.purchasesSubtitle}>Since {formatDate(data.purchases.since_date)}</span>
                </div>
                <div style={styles.purchasesTable}>
                  <table style={styles.table}>
                    <thead><tr style={styles.headerRow}><th style={styles.headerCell}>DATE</th><th style={styles.headerCell}>PATIENT</th><th style={styles.headerCell}>PRODUCT</th><th style={styles.headerCell}>AMOUNT</th><th style={styles.headerCell}>ACTION</th></tr></thead>
                    <tbody>
                      {data.purchases.needs_protocol.map(purchase => (
                        <tr key={purchase.id} style={styles.row}>
                          ...
                          <td style={styles.cell}><strong>{purchase.patient_name || 'Unknown'}</strong></td>
                          <td style={styles.cell}><span style={{ ...styles.categoryBadge, background: getCategoryBadge(purchase.category).color }}>{getCategoryBadge(purchase.category).emoji}</span>{purchase.item_name}</td>
                          <td style={styles.cell}>${(purchase.amount || 0).toFixed(2)}</td>
                          <td style={styles.cell}>
                            <button style={{ ...styles.actionBtn, ...styles.startBtn }} onClick={() => {
                              let patient = patients.find(p => (p.name || '').toLowerCase().trim() === (purchase.patient_name || '').toLowerCase().trim()) || patients.find(p => p.ghl_contact_id === purchase.ghl_contact_id);
                              if (patient) { setSelectedPatient(patient); setPatientSearch(patient.name || `${patient.first_name || ''} ${patient.last_name || ''}`.trim()); setShowPatientDropdown(false); }
                              else { setSelectedPatient(null); setPatientSearch(purchase.patient_name || ''); }
                              ...
                              setStartModal(true);
                            }} title="Start Protocol">+ Start</button>
                            <button style={styles.actionBtn} onClick={() => openGHL(purchase.ghl_contact_id)} title="Open in GHL">GHL</button>
                            <button style={{ ...styles.actionBtn, color: '#dc2626' }} onClick={async () => {
                              if (!confirm(`Delete payment for ${purchase.patient_name}?\n\n${purchase.item_name} - $${purchase.amount}`)) return;
                              try { const res = await fetch(`/api/purchases/${purchase.id}`, { method: 'DELETE' }); const result = await res.json(); if (result.success) { showToast('Payment deleted'); fetchData(); } else showToast(result.error || 'Failed to delete', 'error'); } catch (err) { showToast('Error deleting payment', 'error'); }
            ...
            <div style={styles.categoryFilters}>
              {[{ value: 'all', label: 'All Types', emoji: '📋' }, { value: 'peptide', label: 'Peptide', emoji: '🧬' }, { value: 'weight_loss', label: 'Weight Loss', emoji: '💉' }, { value: 'hrt', label: 'HRT', emoji: '💊' }, { value: 'iv', label: 'IV', emoji: '💧' }, { value: 'hbot', label: 'HBOT', emoji: '🫁' }, { value: 'rlt', label: 'RLT', emoji: '🔴' }, { value: 'other', label: 'Other', emoji: '📁' }].map(cat => <button key={cat.value} style={{ ...styles.categoryTab, ...(categoryFilter === cat.value ? styles.categoryTabActive : {}) }} onClick={() => setCategoryFilter(cat.value)}>{cat.emoji} {cat.label}</button>)}
            </div>
            ...
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
              ...
              {!selectedPatient ? (
                <div style={styles.formGroup}>
                  ...
                  <input type="text" placeholder="Start typing patient name..." value={patientSearch} onChange={(e) => { setPatientSearch(e.target.value); setShowPatientDropdown(true); }} onFocus={() => setShowPatientDropdown(true)} style={styles.formInput} />
                  {showPatientDropdown && filteredPatients.length > 0 && (
                    <div style={styles.searchDropdown}>
                      {filteredPatients.map(p => <div key={p.id} style={styles.searchResult} onClick={() => selectPatient(p)}><div style={{ fontWeight: '500' }}>{p.first_name} {p.last_name || p.name}</div><div style={{ fontSize: '12px', color: '#6b7280' }}>{p.ghl_contact_id ? `GHL: ${p.ghl_contact_id}` : 'No GHL ID'}</div></div>)}
                    </div>
                  )}
                </div>
              ) : (
                <div style={styles.selectedPatientBadge}>
                  <div><div style={{ fontWeight: '600' }}>{selectedPatient.first_name} {selectedPatient.last_name || selectedPatient.name}</div><div style={{ fontSize: '12px', color: '#6b7280' }}>{selectedPatient.ghl_contact_id ? `GHL: ${selectedPatient.ghl_contact_id}` : 'No GHL ID'}</div></div>
                  <button style={styles.changeBtn} onClick={() => setSelectedPatient(null)}>Change</button>
                </div>
              )}
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Protocol Type *</label>
                <select value={protocolType} onChange={(e) => { setProtocolType(e.target.value); setProtocolForm({ ...protocolForm, start_date: protocolForm.start_date }); }} style={styles.formSelect}>
                  <option value="">Select type...</option>
              ...
              <div style={styles.formGroup}>
                ...
                <input type="date" value={protocolForm.start_date || ''} onChange={(e) => setProtocolForm({ ...protocolForm, start_date: e.target.value })} style={styles.formInput} required />
              </div>
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Notes</label>
                <textarea value={protocolForm.notes || ''} onChange={(e) => setProtocolForm({ ...protocolForm, notes: e.target.value })} placeholder="Any additional notes..." style={styles.formTextarea} />
              </div>
              <div style={styles.modalActions}>
                <button style={styles.modalCancelBtn} onClick={closeStartModal}>Cancel</button>
                <button style={styles.modalConfirmBtn} onClick={submitStartProtocol} disabled={submitting || !selectedPatient || !protocolType}>{submitting ? 'Creating...' : 'Start Protocol'}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {logModal && (
          <div style={styles.modalOverlay} onClick={closeLogModal}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={styles.formGroup}><label style={styles.formLabel}>Date *</label><input type="date" value={logForm.date || ''} onChange={(e) => setLogForm({ ...logForm, date: e.target.value })} style={styles.formInput} required /></div>
              {renderLogFields()}
              <div style={styles.formGroup}><label style={styles.formLabel}>Notes</label><textarea value={logForm.notes || ''} onChange={(e) => setLogForm({ ...logForm, notes: e.target.value })} placeholder="Any observations..." style={styles.formTextarea} /></div>
              <div style={styles.modalActions}>
                <button style={styles.modalCancelBtn} onClick={closeLogModal}>Cancel</button>
                <button style={styles.modalConfirmBtn} onClick={submitLog} disabled={submitting}>{submitting ? 'Saving...' : 'Log Activity'}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {renewModal && (
          <div style={styles.modalOverlay} onClick={closeRenewModal}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={styles.formGroup}><label style={styles.formLabel}>Extension Start Date</label><input type="date" value={renewModal.startDate} onChange={(e) => setRenewModal({ ...renewModal, startDate: e.target.value })} style={styles.formInput} /></div>
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Add Days</label>
                <select value={renewModal.duration} onChange={(e) => setRenewModal({ ...renewModal, duration: e.target.value })} style={styles.formSelect}>
              ...
              <div style={styles.modalActions}>
                <button style={styles.modalCancelBtn} onClick={closeRenewModal}>Cancel</button>
                <button style={styles.modalConfirmBtn} onClick={confirmRenew} disabled={renewingId === renewModal.protocol.id}>{renewingId === renewModal.protocol.id ? 'Extending...' : 'Extend Protocol'}</button>
              </div>
            </div>
          </div>
        )}
            ...
            <div style={styles.modal} onClick={e => e.stopPropagation()}>
              ...
              <div style={styles.formGroup}>
                ...
                {editModal.category === 'weight_loss' ? (
                  <select value={editForm.medication || ''} onChange={(e) => setEditForm({ ...editForm, medication: e.target.value })} style={styles.formSelect}>
                    <option value="">Select medication...</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div style={styles.formGroup}>
                  ...
                  {editModal.category === 'weight_loss' ? (
                    <select value={editForm.dose || ''} onChange={(e) => setEditForm({ ...editForm, dose: e.target.value })} style={styles.formSelect}>
                      ...
                      {editForm.medication === 'Semaglutide' ? ['0.25mg', '0.5mg', '1mg', '1.7mg', '2.4mg'].map(d => <option key={d} value={d}>{d}</option>) : ['1mg', '2mg', '3mg', '4mg', '5mg', '6mg', '7mg', '8mg', '9mg', '10mg', '11mg', '12mg', '13mg', '14mg', '15mg'].map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                ...
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Frequency</label>
                  <select value={editForm.frequency || ''} onChange={(e) => setEditForm({ ...editForm, frequency: e.target.value })} style={styles.formSelect}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.formLabel}>Program Duration</label>
                <select value={editForm.program_name || ''} onChange={(e) => setEditForm({ ...editForm, program_name: e.target.value })} style={styles.formSelect}>
              ...
              <div style={styles.formGroup}>
                ...
                <select value={editForm.delivery_method || ''} onChange={(e) => setEditForm({ ...editForm, delivery_method: e.target.value })} style={styles.formSelect}>
                  <option value="">Select...</option>
                  <option value="take_home">Take Home</option>
                  <option value="in_clinic">In Clinic</option>
                </select>
              ...
              {editModal.category === 'weight_loss' && (
                <div style={styles.formGroup}><label style={styles.formLabel}>Starting Weight (lbs)</label><input type="number" step="0.1" value={editForm.starting_weight || ''} onChange={(e) => setEditForm({ ...editForm, starting_weight: e.target.value })} style={styles.formInput} placeholder="e.g. 215.5" /></div>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div style={styles.formGroup}><label style={styles.formLabel}>Start Date</label><input type="date" value={editForm.start_date || ''} onChange={(e) => setEditForm({ ...editForm, start_date: e.target.value })} style={styles.formInput} /></div>
                <div style={styles.formGroup}><label style={styles.formLabel}>Last Refill Date</label><input type="date" value={editForm.last_refill_date || ''} onChange={(e) => setEditForm({ ...editForm, last_refill_date: e.target.value })} style={styles.formInput} /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div style={styles.formGroup}><label style={styles.formLabel}>Total Sessions</label><input type="number" value={editForm.total_sessions || ''} onChange={(e) => setEditForm({ ...editForm, total_sessions: e.target.value })} style={styles.formInput} placeholder="e.g. 10" /></div>
                <div style={styles.formGroup}><label style={styles.formLabel}>Sessions Used</label><input type="number" value={editForm.sessions_used || 0} onChange={(e) => setEditForm({ ...editForm, sessions_used: e.target.value })} style={styles.formInput} /></div>
                <div style={styles.formGroup}>
                  <label style={styles.formLabel}>Status</label>
                  <select value={editForm.status || 'active'} onChange={(e) => setEditForm({ ...editForm, status: e.target.value })} style={styles.formSelect}>
              ...
              <div style={styles.formGroup}><label style={styles.formLabel}>Notes</label><textarea value={editForm.notes || ''} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} style={styles.formTextarea} placeholder="Any additional notes..." /></div>
              <div style={styles.modalActions}>
                <button style={styles.modalCancelBtn} onClick={closeEditModal}>Cancel</button>
                <button style={styles.modalConfirmBtn} onClick={submitEdit} disabled={submitting}>{submitting ? 'Saving...' : 'Save Changes'}</button>
              </div>
            </div>
          </div>
        )}
