...
async function sendSMS(to, message, patientId = null) {
    ...
    if (patient?.ghl_contact_id) {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          console.error(`GHL SMS attempt ${attempt + 1} failed:`, ghlRes.status, errorText);
          if (attempt === 0) {
            await new Promise(r => setTimeout(r, 1000));
            continue;
          }
          // Don't fall through to Twilio — GHL is the primary provider
  ...
  // Fallback: Twilio (for when number is transferred to own Twilio account)
  ...
  if (!accountSid || !authToken || (!fromNumber && !messagingServiceSid)) {
    console.log('No SMS provider configured (GHL or Twilio) — skipping SMS');
