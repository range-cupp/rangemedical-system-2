...
// ============================================
// FIND GHL CONTACT BY PHONE (fallback when stored ID is invalid)
// ============================================

export async function findGHLContactByPhone(phone) {
  if (!phone || !GHL_LOCATION_ID) return null;
  ...
  try {
    const response = await fetch(
      `https://services.leadconnectorhq.com/contacts/?locationId=${GHL_LOCATION_ID}&query=${clean}&limit=1`,
      {
        headers: {
    ...
    if (!response.ok) {
      console.error('GHL contact search error:', response.status);
      return null;
    }

    const data = await response.json();
    if (data.contacts && data.contacts.length > 0) {
      console.log('GHL contact found by phone:', data.contacts[0].id);
      return data.contacts[0].id;
    }
    return null;
  } catch (error) {
    console.error('Error searching GHL contact by phone:', error);
    return null;
  }
}

// ============================================
// RESOLVE GHL CONTACT ID (with phone-based fallback + auto-repair)
// If stored contactId is invalid, search by phone and update patient/protocol
// ============================================

export async function resolveGHLContactId(storedContactId, { phone, patientId, protocolId, supabase } = {}) {
  if (!storedContactId) {
    // No stored ID at all — try phone lookup
    if (phone) {
      const foundId = await findGHLContactByPhone(phone);
      if (foundId && supabase) {
        await autoRepairGHLContactId(foundId, { patientId, protocolId, supabase });
      }
      return foundId;
    }
    return null;
  ...
  // Validate stored contact ID with a lightweight GET
  try {
    const response = await fetch(
      `https://services.leadconnectorhq.com/contacts/${storedContactId}`,
      {
        headers: {
          'Authorization': `Bearer ${GHL_API_KEY}`,
          'Version': '2021-07-28',
    ...
    if (response.ok) {
      return storedContactId; // Stored ID is valid
    }

    // Stored ID is invalid — try phone lookup
    console.warn('GHL contact ID invalid:', storedContactId, '— searching by phone');
    if (phone) {
      const foundId = await findGHLContactByPhone(phone);
      if (foundId && supabase) {
        await autoRepairGHLContactId(foundId, { patientId, protocolId, supabase });
    ...
    console.error('Error validating GHL contact:', error);
    return storedContactId; // On network error, try the stored ID anyway
  }
}

// Auto-repair: update patient + protocol with corrected GHL contact ID
async function autoRepairGHLContactId(correctId, { patientId, protocolId, supabase }) {
  if (!supabase || !correctId) return;

  try {
    if (patientId) {
      await supabase.from('patients').update({ ghl_contact_id: correctId }).eq('id', patientId);
      console.log('Auto-repaired patient ghl_contact_id:', patientId, '→', correctId);
    }
    if (protocolId) {
      await supabase.from('protocols').update({ ghl_contact_id: correctId }).eq('id', protocolId);
      console.log('Auto-repaired protocol ghl_contact_id:', protocolId, '→', correctId);
    }
  } catch (error) {
    console.error('Error auto-repairing GHL contact ID:', error);
  }
}

// ============================================
// SYNC PATIENT PHONE/EMAIL TO GHL (source of truth = patient profile)
// Call before sending SMS to ensure GHL has the correct phone
// ============================================

export async function syncPatientContactToGHL(contactId, { phone, email, firstName, lastName } = {}) {
  ...
  if (phone) {
    ...
    const clean = phone.replace(/\D/g, '');
    body.phone = clean.startsWith('1') ? `+${clean}` : `+1${clean}`;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('GHL Contact Sync error:', response.status, errorText);
      return null;
    }

    console.log('GHL contact synced:', contactId, Object.keys(body).join(', '));
    return await response.json();
  } catch (error) {
    console.error('Error syncing GHL contact:', error);
    return null;
  }
}
...
// ============================================
// SMS TO STAFF (by phone number)
// ============================================

export async function sendStaffSMS(message) {
  // Hardcoded staff phone for alerts
  const staffPhone = '9496900339';
  ...
  try {
    // First, find or create a contact for this phone number
    const searchResponse = await fetch(
      `https://services.leadconnectorhq.com/contacts/?locationId=${GHL_LOCATION_ID}&query=${cleanPhone}&limit=1`,
      {
        headers: {
          'Authorization': `Bearer ${GHL_API_KEY}`,
          'Version': '2021-07-28',
    ...
    // If no contact found, create one for staff alerts
    if (!contactId) {
    ...
    if (!contactId) {
      console.error('Could not find or create staff contact for SMS');
      return null;
    }
    ...
    if (!smsResponse.ok) {
      ...
      console.error('GHL Staff SMS error:', smsResponse.status, errorText);
      return null;
    }

    const result = await smsResponse.json();
    console.log('Staff SMS sent to:', staffPhone);
    return result;
  } catch (error) {
...
export async function addGHLTag(contactId, tagName) {
  if (!contactId || !tagName) {
    console.log('GHL Sync: No contact ID or tag name, skipping tag');
    return null;
  }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('GHL Tag error:', response.status, errorText);
      return null;
    }
    
    const result = await response.json();
    console.log('GHL Tag added for:', contactId, '| Tag:', tagName);
    return result;
  } catch (error) {
...
export async function syncHRTProtocolCreated(contactId, protocol, patientName, options = {}) {
  ...
  const fields = {
    'hrt__program_status': 'Active',
    'hrt__program_start_date': startDate,
    'hrt__current_medication': medication,
    'hrt__current_dosage': dosage,
    'hrt__current_fulfillment': fulfillmentType,
    'hrt__last_medication_pickup': startDate,
    'hrt__next_fulfillment_date': nextFulfillmentDate,
    'hrt__next_blood_draw': nextBloodDraw.toISOString().split('T')[0]
  };
  
  if (secondaryMed) {
    fields['hrt__secondary_medication'] = secondaryMed;
  }
  if (paymentAmount) {
    fields['hrt__current_payment_amount'] = paymentAmount;
  }
...
export async function syncHRTFulfillment(contactId, patientName, options = {}) {
  ...
  const logDate = options.date || new Date().toISOString().split('T')[0];
  const fulfillmentType = options.fulfillmentType || '';
  const medication = options.medication || '';
  const dosage = options.dosage || '';
  
  // Calculate next fulfillment from the log date
  let nextFulfillmentDate = '';
  const baseDate = new Date(logDate);
  if (fulfillmentType.includes('2 Week') || fulfillmentType.includes('2-week')) {
    baseDate.setDate(baseDate.getDate() + 14);
    nextFulfillmentDate = baseDate.toISOString().split('T')[0];
  } else if (fulfillmentType.includes('4 Week') || fulfillmentType.includes('4-week')) {
  ...
  await updateGHLContact(contactId, {
    'hrt__last_medication_pickup': logDate,
    'hrt__next_fulfillment_date': nextFulfillmentDate
  });
...
export async function syncHRTBloodDraw(contactId, patientName, options = {}) {
  ...
  const logDate = options.date || new Date().toISOString().split('T')[0];
  const panelType = options.panel_type || 'Blood Draw';
  const notes = options.notes || null;
  
  // Next blood draw typically 8-12 weeks from blood draw date
  const nextBloodDraw = new Date(logDate);
  nextBloodDraw.setDate(nextBloodDraw.getDate() + 56); // 8 weeks
  
  await updateGHLContact(contactId, {
    'hrt__last_blood_draw': logDate,
    'hrt__next_blood_draw': nextBloodDraw.toISOString().split('T')[0]
  });
  
  let note = `🩸 HRT BLOOD DRAW COMPLETED
...
export async function syncWeightLossProtocolCreated(contactId, protocol, patientName) {
  ...
  // Update custom fields
  await updateGHLContact(contactId, {
    'wl__program_status': 'Active',
    'wl__medication': medication,
    'wl__start_date': startDate,
    'wl__starting_dose': dose,
    'wl__current_dose': dose,
    'wl__fulfillment': deliveryMethod,
    'wl__next_fulfillment_date': nextDueStr,
    'wl__total_injections': String(totalInjections),
    'wl__injections_used': '0',
    'wl__injections_remaining': String(totalInjections)
  });
...
export async function syncWeightLossInjectionLogged(contactId, protocol, log, patientName) {
  ...
  const fields = {
    'wl__current_dose': dose,
    'wl__injections_used': String(injectionsUsed),
    'wl__injections_remaining': String(injectionsRemaining)
  };

  // Update medication if provided
  if (log.medication || protocol.medication) {
    fields['wl__medication'] = log.medication || protocol.medication;
  }
  
  if (currentWeight) {
    fields['wl__current_weight'] = String(currentWeight);
    // Set starting weight if first injection
    if (injectionsUsed === 1) {
      fields['wl__starting_weight'] = String(currentWeight);
    }
  }
  
  if (injectionsRemaining > 0) {
    fields['wl__next_fulfillment_date'] = nextDueStr;
  }
  
  if (injectionsRemaining === 0) {
    fields['wl__program_status'] = 'Completed';
  }
...
Date: ${logDate}
Medication: ${log.medication || protocol.medication || 'N/A'}
Dose: ${dose}`;
...
export async function syncPeptideProtocolCreated(contactId, protocol, patientName) {
  ...
  // Update custom fields
  await updateGHLContact(contactId, {
    'p__peptide_therapy_status': 'Active',
    'p__start_date': startDate,
    'p__peptide': medication,
    'p__peptide_dosage': dose,
    'p_projected_end_date': endDate,
    'p__notes': notes || programName
  });
...
export async function syncPeptideProtocolCompleted(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'p__peptide_therapy_status': 'Completed'
  });
...
export async function syncIVPackageCreated(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'iv__package_name': packageName,
    'iv__package_price': price ? String(price) : '',
    'iv__services': services,
    'iv__quantity': String(totalSessions),
    'iv__used': '0',
    'iv__balance': String(totalSessions)
  });
...
export async function syncIVSessionLogged(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'iv__used': String(sessionsUsed),
    'iv__balance': String(sessionsRemaining)
  });
...
export async function syncInjectionProtocolCreated(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'inj__package_name': packageName,
    'inj__package_price': price ? String(price) : '',
    'inj__quantity': String(totalSessions),
    'inj__used': '0',
    'inj__balance': String(totalSessions)
  });
...
export async function syncInjectionSessionLogged(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'inj__used': String(sessionsUsed),
    'inj__balance': String(sessionsRemaining)
  });
...
export async function syncHBOTPackageCreated(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'hbot__package_name': packageName,
    'hbot__new_package_start_date': startDate,
    'hbot__sessions_in_package': String(totalSessions),
    'hbot__completed_sessions': '0',
    'hbot__sessions_remaining': String(totalSessions)
  });
...
export async function syncHBOTSessionLogged(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'hbot__completed_sessions': String(sessionsUsed),
    'hbot__sessions_remaining': String(sessionsRemaining)
  });
...
export async function syncRLTPackageCreated(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'package_name': packageName, // RLT uses different key format
    'rlt__new_package_start_date': startDate,
    'rlt__sessions_in_package': String(totalSessions),
    'rlt__sessions_used': '0',
    'sessions_remaining': String(totalSessions)
  });
...
export async function syncRLTSessionLogged(contactId, protocol, patientName) {
  ...
  await updateGHLContact(contactId, {
    'rlt__sessions_used': String(sessionsUsed),
    'sessions_remaining': String(sessionsRemaining)
  });
...
export async function syncProtocolToGHL(contactId, protocol, patientName, action = 'created') {
  ...
  if (action === 'created') {
    if (programType.includes('weight_loss') || programName.includes('weight loss')) {
    ...
    } else if (programType.includes('hrt') || programName.includes('hrt')) {
