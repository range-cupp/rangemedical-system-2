  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      if (!wlIsInClinic) {
        ...
        // In-clinic: auto-log first injection (patient is in clinic getting it today)
        const extDose = updateData.selected_dose || protocol.selected_dose || null;
        const { error: wlInClinicErr } = await supabase.from('service_logs').insert({
          patient_id: protocol.patient_id,
          protocol_id: protocol.id,
          category: 'weight_loss',
          ...
          notes: 'Auto-logged from purchase: first injection (in-clinic)',
        });
        if (wlInClinicErr) console.error('Auto service_log error:', wlInClinicErr);
        else {
          updateData.sessions_used = (protocol.sessions_used || 0) + 1;
          console.log(`Auto-protocol: logged weight_loss in-clinic injection for ${protocol.patient_id}`);
        }
      }
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId }) {
  try {
    ...
    // but NO auto-creation, NO auto-extension. Extensions handled at checkout.
    if (protocolType === 'weight_loss') {
      ...
      console.log(`Auto-protocol: weight_loss purchase — linked to ${protocolId || 'none'}`);
      return;
    }
      ...
      if (protocolType === 'weight_loss') {
        ...
        if (fulfillmentMethod === 'in_clinic_injections') {
          ...
          if (wlInClinicErr) console.error('Auto service_log error:', wlInClinicErr);
            ...
            // Increment sessions_used to 1
            await supabase.from('protocols').update({ sessions_used: 1 }).eq('id', protocolId);
            console.log(`Auto-protocol: logged weight_loss in-clinic injection for new protocol ${protocolId}`);
          }
        } else {
