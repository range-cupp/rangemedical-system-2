  ...
  weight_loss: {
    ...
    sections: [
      ...
      },
      {
        ...
        key: 'addons',
        fields: [
          { key: 'addon_injections', label: 'Add-On Vitamins / Injections', type: 'multi_check', options: [
            'None',
            'B12 (Methylcobalamin) 1mg IM',
            'B12 (Cyanocobalamin) 1mg IM',
            'Lipo-C (MIC + B12) IM',
            'Lipo-B (MIC + B Complex) IM',
            'Glutathione 200mg IV Push',
            'Biotin 5mg IM',
            'Vitamin D3 50,000 IU IM',
          ], defaultChecked: [0] },
          { key: 'addon_site', label: 'Add-On Injection Site', type: 'button_group', options: INJECTION_SITES, showIf: (data) => data?.addons?.addon_injections?.length > 0 && !data?.addons?.addon_injections?.includes('None') },
          { key: 'addon_notes', label: 'Add-On Notes', type: 'text', placeholder: 'e.g. Patient requested B12 for energy' },
        ],
      },
      {
  ...
  iv_therapy: {
    ...
    sections: [
      ...
      },
      {
        ...
        key: 'im_addon',
        fields: [
          { key: 'im_injections', label: 'IM Injections', type: 'multi_check', options: [
            'None',
            'B12 (Methylcobalamin) 1mg IM',
            'B12 (Cyanocobalamin) 1mg IM',
            'Thiamine/Pyridoxine (B1/B6) — 20/100 mg/mL IM',
            'Vitamin D3 50,000 IU IM',
            'Lipo-C (MIC + B12) IM',
            'Lipo-B (MIC + B Complex) IM',
            'Glutathione 200mg IM',
            'Biotin 5mg IM',
            'L-Carnitine 500mg IM',
            'Toradol (Ketorolac) 30mg IM',
          ], defaultChecked: [0] },
          { key: 'im_site', label: 'IM Injection Site', type: 'button_group', options: INJECTION_SITES, showIf: (data) => data?.im_addon?.im_injections?.length > 0 && !data?.im_addon?.im_injections?.includes('None') },
          { key: 'im_notes', label: 'IM Injection Notes', type: 'text', placeholder: 'e.g. Patient requested B12 for energy', showIf: (data) => data?.im_addon?.im_injections?.length > 0 && !data?.im_addon?.im_injections?.includes('None') },
        ],
      },
      {
        title: 'Vitals',
...
function generateWeightLossNote(data, vitals) {
  ...
  const addonList = (addons.addon_injections || []).filter(a => a !== 'None');
  if (addonList.length > 0) {
    lines.push(`**Add-On Injections:** ${addonList.join(', ')}`);
...
function generateIVNote(data) {
  ...
  const activeIM = imInjections.filter(i => i !== 'None');
  if (activeIM.length > 0) {
    lines.push('**IM Injection Add-On:**');
    activeIM.forEach(inj => lines.push(`- ${inj}`));
