...
//   - HRT: ANY change (increase or decrease) requires approval. Provider
//     sign-off is required either way for legal/audit reasons.
...
import { canApproveDoseChange } from './staff-config';
...
export async function guardDoseChange(supabase, currentProtocol, update, opts = {}) {
  const { mode = 'reject', approvedRequestId, userEmail } = opts;

  // Providers authorized to approve dose changes can save directly.
  if (userEmail && canApproveDoseChange(userEmail)) {
    return { allowed: true, sanitizedUpdate: update, providerBypass: true, providerEmail: userEmail };
  }
  ...
  // Cancelling or completing a protocol is not a clinical dose change. The
  // edit modal sends every field on save, so a status flip to cancelled/
  // completed would otherwise trip this guard if the form's dose values
  // round-trip differently than what's in the DB (e.g. injections_per_week
  // defaulting to 2 when the DB has null). The protocol won't be
  // administered at any new dose once it's closed, so there's nothing to gate.
  ...
  const reason = isWL
    ? 'Weight-loss dose changes require provider approval. Use the Change Dose action on the patient profile to send an approval request to Dr. Burgess or Brendyn Reed NP.'
    : 'HRT dose changes require provider approval. Use the Change Dose action on the patient profile to send an approval request to Dr. Burgess or Brendyn Reed NP.';
