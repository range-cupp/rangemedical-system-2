...
export async function postImportActions(supabase, patientId, patientName, testDate, reviewerIds) {
  ...
  // ── 5. Pipeline card + single joint review task ───────────────────────────
  // Route to the right pipeline based on whether the patient is already on an
  // active treatment plan: HRT/WL patients land on follow_up_labs, everyone
  // else goes on Main Pipeline (energy_workup). Skip generic task creation —
  // we insert the rich joint task below.
  let pipelineCardId = null;
  let targetPipeline = 'energy_workup';
  try {
    const { data: activeTx } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (activeTx) targetPipeline = 'follow_up_labs';
  } catch (e) {
    console.error(`[lab-post-import] Active treatment lookup error for ${patientName}:`, e.message);
  }

  try {
    const { ensureLabsCardAtUnderReview } = await import('./pipeline-automations');
    const card = await ensureLabsCardAtUnderReview({
      patientId,
      pipelineKey: targetPipeline,
      reason: 'lab_biomarker_import',
      skipTaskCreation: true,
    });
    pipelineCardId = card?.id || null;
  } catch (e) {
    console.error(`[lab-post-import] Pipeline card ensure error for ${patientName}:`, e.message);
  }

  // ONE joint review task for Damien Burgess + Evan. Completing it marks the
  // task done for both — no duplicate work. Primary assignee = Burgess (or
  // Evan if Burgess missing); the other rides along in additional_assignees
  // so both see it on their task lists.
  let primaryAssigneeId = burgessId || evanId;
  const additionalAssignees = (burgessId && evanId) ? [evanId] : [];
  ...
  if (primaryAssigneeId) {
    ...
    try {
      ...
      // SMS both reviewers (primary + additional) so each gets the heads-up.
      const { notifyTaskAssignee } = await import('./notify-task-assignee');
      const recipients = [primaryAssigneeId, ...additionalAssignees];
      for (const id of recipients) {
        notifyTaskAssignee(id, {
          assignerName: 'Lab Import',
          taskTitle: reviewTask.title,
          priority: 'high',
        }).catch(err => console.error('[lab-post-import] Task SMS error:', err.message));
      }
    } catch (e) {
