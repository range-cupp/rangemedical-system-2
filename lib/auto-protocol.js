...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}, fulfillmentOpts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'hrt': {
      ...
      // Supply tracking: when operator dispensed N prefilled syringes today,
      // compute days-of-supply from the injection cadence so the system knows
      // when the patient runs out (next_expected_date) and the 7-day-prior
      // follow-up cron can flag the refill.
      ...
      const supplyDays = dispenseQty
        ...
        : 30; // vials default to ~30 day cycle for billing purposes
      ...
      return {
        ...
        ...(dispenseQty ? {
          ...
          supply_quantity: dispenseQty,
          supply_remaining: dispenseQty,
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId, wlFrequencyDays, wlConfig, peptideConfig, hrtConfig, injectionFrequency }) {
  try {
    ...
    if (existing) {
      ...
      // HRT: when the operator dispensed pre-filled syringes at checkout via the
      // HRT Builder modal, log the pickup so the supply countdown / pickup
      // history is accurate. Vials and config-less HRT signups still rely on
      // the Dispense button on the patient page.
      if (protocolType === 'hrt') {
