...
async function handleCancelAppointment(params) {
  try {
    ...
    // 2. Cancel both forms of the linked calcom_bookings row so the reminder
    // cron stops texting (matches /api/appointments/[id]/cancel logic).
    {
      const cancelledAt = new Date().toISOString();
      const updates = { status: 'cancelled', cancelled_at: cancelledAt };
      if (booking.cal_com_booking_id) {
        const calIdInt = parseInt(booking.cal_com_booking_id, 10);
        if (!Number.isNaN(calIdInt)) {
          await supabase.from('calcom_bookings').update(updates).eq('calcom_booking_id', calIdInt);
        }
      }
      await supabase.from('calcom_bookings').update(updates).eq('calcom_uid', `local-${booking.id}`);
    }
...
async function handleRescheduleAppointment(params) {
  try {
    ...
    // Cancel both forms of the linked calcom_bookings row.
    {
    ...
    // Shadow row for the new appointment.
    try {
      const { toPacificDate } = await import('./date-utils');
      ...
      console.error('staff-bot reschedule shadow row error (non-fatal):', e);
    }
