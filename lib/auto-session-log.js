...
import { createServiceLogEntry } from './service-log-engine';
...
export async function autoLogSessionFromAppointment(appointment) {
  ...
  // Use the appointment's actual Pacific date as the visit date — never
  // "today". A completion processed late (e.g., next morning, or while
  // editing an old appointment) must still log to the day the visit
  // happened, otherwise the dup check fails and a phantom log is created.
  const apptDate = appointment.start_time
    ? new Date(appointment.start_time).toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' })
    : todayPacific();
  ...
  // Log via central engine (handles insert, billing flag, recount, weight sync)
  ...
  if (engineErr) {
    console.error('Auto-session-log engine error:', engineErr);
    return;
  }

  // Update scheduling fields on the protocol (recount already done by engine)
  if (protocol) {
    ...
    console.log(`Auto-session-log: logged ${serviceType.category} ${serviceType.entryType} for patient ${patientId}, protocol ${protocol.id} (${provider || 'no provider'})`);
  } else {
