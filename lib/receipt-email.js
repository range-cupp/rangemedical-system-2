...
export function generateReceiptHtml({
  ...
  // For single-item receipts, check for discount
  ...
  // Build line items HTML for multi-item receipts
  let lineItemsHtml = '';
  if (isMultiItem) {
    lineItemsHtml = items.map(item => {
      ...
      const originalLine = hasItemDiscount ? `$${((item.unitPriceCents * qty) / 100).toFixed(2)}` : null;

      return `
                                            <tr>
                                                <td style="padding: 6px 0; color: #111; font-size: 14px;">${nameWithQty}${item.discountLabel ? ` <span style="color: #16A34A; font-size: 12px;">(${item.discountLabel})</span>` : ''}</td>
                                                <td style="padding: 6px 0; color: #111; font-size: 14px; text-align: right;">
                                                    ${hasItemDiscount ? `<span style="text-decoration: line-through; color: #999; margin-right: 6px; font-size: 12px;">${originalLine}</span>` : ''}${lineTotal}
                                                </td>
                                            </tr>`;
    }).join('');
  }
                    ...
                    <tr>
                        <td style="padding: 40px 30px;">
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 0 0 10px;">
                                <tr>
                                    <td style="border-left: 4px solid #000000; padding-left: 20px;">
                                        <p style="margin: 0 0 8px; color: #888; font-size: 11px; text-transform: uppercase; font-weight: 600; letter-spacing: 0.05em;">Items</p>
                                        <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                            ${lineItemsHtml}
                                        </table>
                                    </td>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin: 0 0 30px;">
                                <tr>
                                    <td align="center">
                                        <a href="${process.env.NEXT_PUBLIC_BASE_URL || 'https://app.range-medical.com'}/api/receipt/${invoiceId}" style="display: inline-block; background-color: #000000; color: #ffffff; padding: 12px 28px; border-radius: 6px; font-size: 14px; font-weight: 600; text-decoration: none;">Download Receipt PDF</a>
                                    </td>
                                </tr>
                            </table>
