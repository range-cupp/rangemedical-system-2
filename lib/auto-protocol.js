...
import { findPeptideInfo, WEIGHT_LOSS_DOSAGES, findPeptideProduct, parseDurationFromName, parseDeliveryFromName, parsePhaseFromName, isGHPeptide, isRecoveryPeptide, getCycleConfig, calculatePeptideDurationDays, calculateProtocolDurationDays, buildSig, buildWeightLossSig, buildPeptideSig } from './protocol-config';
...
import { spawnTakeHomeInjections } from './spawn-takehome-injections';
...
// ================================================================
// Parse WL frequency from service name: "14-day"/"biweekly" → 14, "10-day" → 10, "monthly" → 7
function parseWlFrequency(serviceName, explicitFrequency) {
  if (explicitFrequency) return parseInt(explicitFrequency, 10) || 7;
  const lower = (serviceName || '').toLowerCase();
  if (lower.includes('14-day') || lower.includes('14 day') || lower.includes('every 14') || lower.includes('biweekly') || lower.includes('bi-weekly')) return 14;
  if (lower.includes('10-day') || lower.includes('10 day') || lower.includes('every 10')) return 10;
  return 7; // default weekly
}

// Map a WL frequency in days → human label stored on the protocol record.
function wlFrequencyLabel(days) {
  if (days === 7) return 'Weekly';
  if (days === 14) return 'Every 14 Days';
  return `Every ${days} Days`;
}

// Notes phrase for service log entries — "weekly" / "every N days".
function wlFrequencyNote(days) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      return {
        ...
        frequency: wlFrequencyLabel(frequencyDays),
        sig: buildWeightLossSig(parsedDose),
    ...
    case 'hrt': {
      // hrtConfig (from the checkout HRT Builder modal) is authoritative when
      // present — operator picked medication/dose/frequency/supply explicitly,
      // so we skip the hardcoded fallbacks and clear the review flag.
      const cfg = opts.hrtConfig;
      const gender = (cfg?.hrtType || opts.patientGender || 'male').toLowerCase();
      const isFemale = gender === 'female';

      const dose = cfg?.selectedDose || (isFemale ? '0.25ml/25mg' : '0.5ml/100mg');
      const frequency = cfg?.frequency || (isFemale ? 'Weekly' : 'Every 3.5 days');
      const route = cfg?.route || 'Intramuscular';
      const form = cfg?.form || 'Solution';
      const sig = cfg?.sig || buildSig({ dose, route, frequency, form });
      const medication = cfg?.medication || getHRTMedication(gender);

      // Map the human-readable frequency to the legacy enum the rest of the
      // system reads. Cypionate every-3.5-days is the default.
      const freqLower = (frequency || '').toLowerCase();
      let freqEnum = 'every_3_5_days';
      let injectionsPerWeek = 2;
      if (freqLower.includes('weekly') && !freqLower.includes('twice') && !freqLower.includes('bi')) { freqEnum = 'weekly'; injectionsPerWeek = 1; }
      else if (freqLower.includes('twice')) { freqEnum = 'twice_weekly'; injectionsPerWeek = 2; }
      else if (freqLower.includes('every 2 week') || freqLower.includes('biweekly') || freqLower.includes('bi-weekly')) { freqEnum = 'biweekly'; injectionsPerWeek = 0.5; }
      else if (freqLower.includes('3x')) { injectionsPerWeek = 3; }

      // Parse "0.3ml/60mg" → { ml: 0.3, mg: 60 } for the dose tracking columns
      const doseMatch = (dose || '').match(/^([\d.]+)ml\/([\d.]+)mg$/i);
      const doseMl = doseMatch ? parseFloat(doseMatch[1]) : null;
      const doseMgVal = doseMatch ? parseFloat(doseMatch[2]) : null;

      // Supply tracking: compute days-of-supply from the injection cadence so
      // the system knows when the patient runs out (next_expected_date) and
      // the 7-day-prior follow-up cron can flag the refill.
      const isVialSupply = cfg?.supplyType === 'vial_10ml' || cfg?.supplyType === 'vial_5ml' || cfg?.supplyType === 'vial';
      const dispenseQty = cfg?.dispenseQty && cfg.supplyType === 'prefilled'
        ? Math.max(1, parseInt(cfg.dispenseQty) || 0)
        : null;
      const vialMl = isVialSupply ? (cfg.supplyType === 'vial_5ml' ? 5 : 10) : 0;
      const supplyDays = dispenseQty
        ? Math.max(1, Math.round(dispenseQty * 7 / injectionsPerWeek))
        : isVialSupply && doseMl > 0
          ? Math.max(1, Math.round((vialMl / (doseMl * injectionsPerWeek)) * 7))
          : 30;
      const hrtEnd = new Date();
      hrtEnd.setDate(hrtEnd.getDate() + supplyDays);

      return {
        ...base,
        program_name: 'HRT Protocol',
        medication,
        hrt_type: isFemale ? 'female' : 'male',
        selected_dose: dose,
        starting_dose: dose,
        current_dose: dose,
        frequency: freqEnum,
        injection_method: route === 'Subcutaneous' ? 'subq' : 'im',
        sig,
        delivery_method: cfg?.deliveryMethod || 'take_home',
        end_date: hrtEnd.toISOString().split('T')[0],
        ...(cfg?.supplyType ? { supply_type: cfg.supplyType, fulfillment_type: cfg.supplyType } : {}),
        ...((dispenseQty || isVialSupply) ? {
          last_refill_date: today,
          supply_dispensed_date: today,
          supply_days: supplyDays,
          next_expected_date: hrtEnd.toISOString().split('T')[0],
          injections_per_week: injectionsPerWeek,
          injection_frequency: String(injectionsPerWeek),
          ...(doseMl !== null ? { dose_per_injection: String(doseMl) } : {}),
          ...(doseMgVal !== null ? { dose_mg: doseMgVal, vial_mg: doseMgVal } : {}),
          ...(dispenseQty ? { supply_quantity: dispenseQty, supply_remaining: dispenseQty } : {}),
        } : {}),
        ...(cfg?.secondaryMedications?.length ? {
          secondary_medications: JSON.stringify(cfg.secondaryMedications.map(s => s.medication)),
          secondary_medication_details: cfg.secondaryMedications.map(s => ({
            ...
            sig: buildSig({ dose: s.dose, route: s.route, frequency: s.frequency, form: s.form }),
          })),
        } : {}),
        // Builder-created protocols are operator-verified at checkout — no review
        // task. Auto-created (no cfg) protocols still require provider review.
        needs_medication_review: !cfg,
        ...(cfg ? { medication_verified_at: new Date().toISOString() } : {}),
        // Used downstream by autoCreateOrExtendProtocol to log the dispense
        // service_log without touching the DB columns above.
        _hrtConfig: cfg || null,
      };
    }
    ...
    case 'hbot': {
      ...
      const hbotSessions = sessions > 1 ? sessions : (opts.quantity || 1);
      const endDate = new Date();
      endDate.setDate(endDate.getDate() + hbotSessions * 7);
      return {
        ...base,
        program_name: hbotSessions > 1 ? `HBOT - ${hbotSessions} Pack` : 'HBOT Session',
    ...
    case 'rlt': {
      ...
      const rltSessions = sessions > 1 ? sessions : (opts.quantity || 1);
      const endDate = new Date();
      endDate.setDate(endDate.getDate() + rltSessions * 7);
      return {
        ...base,
        program_name: rltSessions > 1 ? `Red Light Therapy - ${rltSessions} Pack` : 'Red Light Therapy',
    ...
    case 'peptide': {
      ...
      if (isVialPurchase) {
        const vialFreq = peptideInfo?.frequency || null;
        return {
          ...base,
...
async function findExistingProtocol(patientId, protocolType, medication) {
  ...
  // Include recently expired protocols for take-home types
  // WL patients routinely go overdue (5-12 weeks); 90-day window prevents
  // duplicate protocols when they finally refill
  if (['weight_loss', 'hrt', 'peptide'].includes(protocolType)) {
    const lookbackDays = protocolType === 'weight_loss' ? 90 : 30;
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - lookbackDays);
    query = query.in('status', ['active', 'in_progress', 'expired', 'completed'])
      .or(`end_date.is.null,end_date.gte.${cutoff.toISOString().split('T')[0]}`);
  } else {
...
// ================================================================
async function extendProtocol(protocol, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays, wlConfig, peptideConfig, hrtConfig, injectionFrequency } = {}) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      // total_sessions = doses paid for. A new purchase adds its quantity,
      // period. Don't max() against sessions_used — that was a workaround
      // for a long-fixed drift bug, and it actively inflates the total
      // whenever a patient buys more after going past their prepaid block
      // (Claudia: 13 used / 12 paid + 4-pack would land at 17 instead of 16).
      ...
      updateData.frequency = wlFrequencyLabel(wlFreqDays);
      ...
      // Keep sig in sync with the latest dose so patient-facing rx directions
      // never show a stale dose after a titration extend.
      const newWlDose = updateData.selected_dose || protocol.selected_dose;
      if (newWlDose) updateData.sig = buildWeightLossSig(newWlDose);

      // Auto-log pickup in service_logs — per group for wlConfig, single entry for legacy.
      // Each take-home pickup also spawns one injection row per dose dispensed
      // (dated for the patient's injection_day) so sessions_used reflects what
      // they're paid up to take, not just what they've come in for.
      if (cfg?.groups?.length) {
        for (const group of cfg.groups) {
          if (group.fulfillment === 'in_clinic') continue; // in-clinic logs at appointment
          const { data: pickupRow, error: slErr } = await supabase
            .from('service_logs')
            .insert({
              ...
              notes: `Auto-logged from purchase: ${group.quantity} × ${group.dose} dispensed (${wlFrequencyNote(wlFreqDays)})`,
            })
            .select()
            .single();
          if (slErr) { console.error('Auto service_log error:', slErr); continue; }
          await spawnTakeHomeInjections(supabase, pickupRow, { ...protocol, ...updateData });
        }
        ...
        const extDosageLabel = wlInjections > 1
          ...
          : extDose;
        const { data: pickupRow, error: wlSlErr } = await supabase
          .from('service_logs')
          .insert({
            patient_id: protocol.patient_id,
            protocol_id: protocol.id,
        ...
        if (wlSlErr) console.error('Auto service_log error:', wlSlErr);
          ...
          console.log(`Auto-protocol: logged weight_loss pickup for ${protocol.patient_id} (${wlInjections} injections)`);
          await spawnTakeHomeInjections(supabase, pickupRow, { ...protocol, ...updateData });
        }
      }
    ...
    case 'hrt': {
      const hrtCfg = hrtConfig;
      const hrtIsVial = hrtCfg?.supplyType === 'vial_10ml' || hrtCfg?.supplyType === 'vial_5ml' || hrtCfg?.supplyType === 'vial';
      const hrtIsPrefilled = hrtCfg?.supplyType === 'prefilled' && hrtCfg?.dispenseQty;

      if (hrtIsVial) {
        const hrtVialMl = hrtCfg.supplyType === 'vial_5ml' ? 5 : 10;
        const doseMl = parseFloat(protocol.dose_per_injection) || parseFloat((hrtCfg?.selectedDose || '').match(/^([\d.]+)ml/i)?.[1]) || 0.4;
        const ipw = parseInt(protocol.injections_per_week) || parseInt(protocol.injection_frequency) || 2;
        const vialSupplyDays = doseMl > 0 ? Math.max(1, Math.round((hrtVialMl / (doseMl * ipw)) * 7)) : 70;
        const vialEnd = new Date(today);
        vialEnd.setDate(vialEnd.getDate() + vialSupplyDays);
        updateData.last_refill_date = todayStr;
        updateData.next_expected_date = vialEnd.toISOString().split('T')[0];
        updateData.supply_type = hrtCfg.supplyType;
        updateData.supply_days = vialSupplyDays;
        const currentEndDate2 = protocol.end_date ? new Date(protocol.end_date + 'T12:00:00') : today;
        if (vialEnd > currentEndDate2) updateData.end_date = vialEnd.toISOString().split('T')[0];
        else {
          const newEnd = new Date(startFrom);
        ...
        await supabase.from('service_logs').insert({
          ...
          entry_type: 'pickup',
          entry_date: todayStr,
          medication: hrtCfg.medication || protocol.medication || null,
          dosage: hrtCfg.selectedDose || protocol.selected_dose || null,
          quantity: 1,
          supply_type: hrtCfg.supplyType,
          fulfillment_method: fulfillmentMethod || 'in_clinic',
          notes: `Auto-logged from HRT checkout: ${hrtVialMl}ml vial dispensed.`,
        });
        console.log(`Auto-protocol: logged HRT vial pickup (${hrtVialMl}ml) for protocol ${protocol.id}, supply ${vialSupplyDays} days`);
      } else {
        const newEnd = new Date(startFrom);
        newEnd.setDate(newEnd.getDate() + 30);
        updateData.end_date = newEnd.toISOString().split('T')[0];
    ...
    case 'peptide': {
      ...
      if (protocol.doses_per_vial) {
        ...
        // Phase-based peptides (2X/3X/4X blends) store doses in a `phases` array
        // keyed by phase number — pull the phase from the service name / builder
        // config and use that phase's dose so the protocol's dose tracks the phase
        // the patient was just charged for.
        const extendPhase = pepCfg?.phase || parsePhaseFromName(serviceName);
        if (pepCatalog?.phases?.length) {
          const phaseData = extendPhase
            ? pepCatalog.phases.find(p => p.phase === extendPhase)
            : null;
          if (phaseData?.dose) {
            updateData.selected_dose = phaseData.dose;
            updateData.current_dose = phaseData.dose;
          }
        } else if (pepCatalog?.dose) {
          updateData.selected_dose = pepCatalog.dose;
          updateData.current_dose = pepCatalog.dose;
        }
      }
...
// ================================================================
// AUTO-SEND PEPTIDE GUIDE
// Triggered after a new peptide protocol is created from a purchase.
// Reuses /api/admin/protocols/[id]/send-guide so guide content stays in
// one place (vial vs prefilled, days, dose, frequency, Blooio opt-in).
// Failures are non-fatal — the purchase has already succeeded.
// ================================================================
async function autoSendPeptideGuide(protocolId) {
  if (!protocolId) return;
  try {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://app.range-medical.com';
    const res = await fetch(`${baseUrl}/api/admin/protocols/${protocolId}/send-guide`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ source: 'auto-protocol' }),
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      console.error(`Auto-send peptide guide failed for protocol ${protocolId}:`, data.error || res.statusText);
    } else {
      console.log(`Auto-send peptide guide triggered for protocol ${protocolId}`);
    }
  } catch (err) {
    console.error(`Auto-send peptide guide error for protocol ${protocolId}:`, err.message);
  }
}
...
async function handlePeptideSequentialProtocol({
  ...
  trackingNumber, quantity, serviceCategory, peptideConfig,
}) {
  const today = todayPacific();
  const catalogMatch = peptideConfig?.medication
    ? findPeptideProduct(peptideConfig.medication) || findPeptideProduct(serviceName)
    : findPeptideProduct(serviceName);
  const peptideName = catalogMatch?.medication || medication || serviceName;
  const duration = peptideConfig?.durationDays || durationDays || parseDurationFromName(serviceName) || catalogMatch?.duration || 30;
  ...
  const buildPeptideProtocol = (startDate, endDate, status, continuousDaysStarted) => {
    const method = peptideConfig?.hasTakeHome ? 'take_home'
      : peptideConfig?.hasInClinic ? 'in_clinic'
  ...
  try {
    ...
    if (isVialPurchase) {
      // Pull clinical facts (injections-per-vial, frequency) from VIAL_CATALOG
      // via the catalog entry's vialId. This is the single source of truth —
      // every duration math call derives from these numbers, so protocol dates
      // cannot silently drift from what the patient actually has in hand.
      const vialCatalogId = catalogMatch?.vialId || null;
      const vialInfo = vialCatalogId ? VIAL_CATALOG.find(v => v.id === vialCatalogId) : null;
      const dosesPerVial = (() => {
        const raw = vialInfo?.injectionsPerVial;
      ...
      const vialFrequency = vialInfo?.frequency || catalogMatch?.defaultFrequency || 'Daily';
      // Snapshot end-date derivation (kept consistent below + on extensions):
      //   total doses = num_vials × doses_per_vial
      //   end_date   = start_date + calculatePeptideDurationDays(totalDoses, frequency)
      const endDateFromVials = (numVials, startDateStr) => {
        const totalDoses = dosesPerVial ? numVials * dosesPerVial : null;
        const days = totalDoses ? calculatePeptideDurationDays(totalDoses, vialFrequency) : null;
      ...
      if (vialMatch) {
        ...
        const effectiveDpv = vialMatch.doses_per_vial || dosesPerVial;
        const effectiveFreq = vialMatch.frequency || vialFrequency;
        const newEndDate = (() => {
          const totalDoses = effectiveDpv ? newNumVials * effectiveDpv : null;
          const days = totalDoses ? calculatePeptideDurationDays(totalDoses, effectiveFreq) : null;
          if (!days) return vialMatch.end_date;
          const start = vialMatch.start_date || today;
        ...
        await logWebhookAction(purchaseId, 'peptide_vial_added', {
          protocol_id: vialMatch.id, medication: peptideName, added_vials: vialQty, total_vials: newNumVials, new_end_date: newEndDate,
        });
        console.log(`Auto-protocol: added ${vialQty} vial(s) to ${peptideName} protocol ${vialMatch.id} (total: ${newNumVials}, end: ${newEndDate})`);
      ...
      const endDate = endDateFromVials(vialQty, today);
      ...
      const vialProtocolData = {
        ...
        program_type: 'peptide',
        medication: peptideName,
        selected_dose: peptideInfo?.startingDose || catalogMatch?.dose || vialInfo?.dosage || null,
        frequency: vialFrequency,
        delivery_method: method,
        num_vials: vialQty,
        // Snapshot clinical fact so downstream displays (Preview Guide, patient
        // page duration) don't re-run catalog lookups and risk drift.
      ...
      await logWebhookAction(purchaseId, 'peptide_vial_protocol_created', {
        protocol_id: createResult.protocol.id, medication: peptideName, num_vials: vialQty, doses_per_vial: dosesPerVial, end_date: endDate,
      });
      console.log(`Auto-protocol: created vial protocol ${createResult.protocol.id} (${peptideName}, ${vialQty} vial${vialQty > 1 ? 's' : ''}, ${dosesPerVial ? vialQty * dosesPerVial + ' doses' : '?'}, end ${endDate})`);

      await autoSendPeptideGuide(createResult.protocol.id);
    ...
    if (!activeProtocol && !queuedProtocol) {
      ...
      await autoSendPeptideGuide(newProto.id);
    }
...
// ================================================================
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId, wlFrequencyDays, wlConfig, peptideConfig, hrtConfig, injectionFrequency, staffUserId, staffUserName }) {
  try {
    ...
    // Look up patient name + gender for the protocol record. Gender drives the
    // HRT defaults (medication concentration, dose, frequency).
    let patientName = null;
    let patientGender = null;
    if (patientId) {
      const { data: pt } = await supabase
        .from('patients')
        .select('name, full_name, gender')
        .eq('id', patientId)
        .single();
    ...
    // 1. Builder peptideConfig.medication is authoritative (set by Peptide Builder)
    // 2. Try catalog match from serviceName
    // 3. Fall back to parsing for legacy/old-format names
    if (protocolType === 'peptide') {
      if (peptideConfig?.medication) {
        const catalogMatch = findPeptideProduct(peptideConfig.medication);
        medication = catalogMatch?.medication || peptideConfig.medication;
      } else if (serviceName) {
    ...
    // ── Duplicate-WL guard ──
    // findExistingProtocol matches WL by exact medication, so a Tirzepatide
    // SKU rung up on a Retatrutide patient returns null and would otherwise
    // fall through to create a duplicate WL protocol on the wrong drug
    // (Lauren Lopez-Galvez, 2026-04-30). Refuse the create when the patient
    // already has an active WL protocol on a different medication. The
    // purchase stays orphaned and surfaces in the repair-orphaned-purchases
    // flow for staff to reconcile manually.
    if (!existing && protocolType === 'weight_loss' && medication) {
      const { data: otherActiveWL } = await supabase
        .from('protocols')
        .select('id, medication, selected_dose')
        .eq('patient_id', patientId)
        .eq('program_type', 'weight_loss')
        .in('status', ['active', 'in_progress'])
        .not('medication', 'is', null);

      const conflicting = (otherActiveWL || []).filter(p =>
        p.medication && p.medication.toLowerCase() !== medication.toLowerCase()
      );

      if (conflicting.length > 0) {
        const conflictMeds = conflicting.map(p => p.medication).join(' / ');
        console.error(
          `[auto-protocol] BLOCKED duplicate WL protocol: patient ${patientId} ` +
          `already has active ${conflictMeds}, but purchase ${purchaseId} is for ${medication}. ` +
          `Leaving purchase orphaned — staff must reconcile via /api/admin/repair-orphaned-purchases.`
        );
        await logAction({
          employeeId: staffUserId || null,
          employeeName: staffUserName || 'Unknown (no staff context)',
          action: 'block_duplicate_wl_protocol',
          resourceType: 'purchase',
          resourceId: purchaseId || null,
          details: {
            patient_id: patientId,
            patient_name: patientName,
            purchase_id: purchaseId || null,
            sku_medication: medication,
            existing_active_medications: conflicting.map(p => ({
              protocol_id: p.id,
        ...
        }).catch(err => console.error('audit_log block_duplicate_wl_protocol failed:', err));
        return;
      }
    }
    ...
    if (existing) {
      ...
      protocolId = await extendProtocol(existing, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays, wlConfig, peptideConfig, hrtConfig, injectionFrequency });
      console.log(`Auto-protocol: extended ${protocolType} protocol ${protocolId} for patient ${patientId}`);
    } else {
      // Create new protocol
      const protocolData = buildProtocolData(protocolType, serviceName, patientId, { deliveryMethod, durationDays, medication, patientName, patientGender, serviceCategory, quantity, wlFrequencyDays, wlConfig, peptideConfig, hrtConfig, injectionFrequency }, { fulfillmentMethod });
      if (!protocolData) return;

      // Strip _wlConfig / _peptideConfig / _hrtConfig before DB insert — used for service log creation only
      const { _wlConfig: _wlCfgLocal, _peptideConfig: _pepCfgLocal, _hrtConfig: _hrtCfgLocal, ...dbProtocolData } = protocolData;
      ...
      if (protocolType === 'weight_loss') {
        ...
        if (newWlCfg?.groups?.length) {
          ...
          for (const group of newWlCfg.groups) {
            ...
            const { data: pickupRow, error: slErr } = await supabase
              ...
              .insert({
                ...
                fulfillment_method: fulfillmentMethod || 'in_clinic',
                tracking_number: trackingNumber || null,
                notes: `Auto-logged from purchase: ${group.quantity} × ${group.dose} dispensed (${wlFrequencyNote(newWlFreqDays)})`,
              })
              .select()
              .single();
            if (slErr) { console.error('Auto service_log error:', slErr); continue; }
            await spawnTakeHomeInjections(supabase, pickupRow, protocolData);
          }
          ...
          const wlDoseMatch = (serviceName || '').match(/(\d+(?:\.\d+)?)\s*mg/i);
          const supplyLabel = newWlFreqDays === 7
            ? `${injCount} week supply`
            : `${injCount * newWlFreqDays} day supply (${wlFrequencyNote(newWlFreqDays)})`;
          ...
          const { data: pickupRow, error: wlNewSlErr } = await supabase
            .from('service_logs')
            .insert({
              patient_id: patientId,
              protocol_id: protocolId,
              ...
              fulfillment_method: fulfillmentMethod || 'in_clinic',
              tracking_number: trackingNumber || null,
              notes: `Auto-logged from purchase: ${injCount} injection${injCount > 1 ? 's' : ''} dispensed (${wlFrequencyNote(newWlFreqDays)})`,
            })
            .select()
            .single();
          if (wlNewSlErr) console.error('Auto service_log error:', wlNewSlErr);
          else if (pickupRow) await spawnTakeHomeInjections(supabase, pickupRow, protocolData);
      ...
      // HRT: log pickup when operator dispensed supply at checkout (prefilled
      // syringes or vials). This drives the supply countdown and pickup history.
      if (protocolType === 'hrt') {
        const hrtCfg = protocolData._hrtConfig;
        const hrtPickupQty = hrtCfg?.dispenseQty && hrtCfg.supplyType === 'prefilled'
          ? Math.max(1, parseInt(hrtCfg.dispenseQty) || 0)
          : 0;
        const hrtIsVialNew = hrtCfg?.supplyType === 'vial_10ml' || hrtCfg?.supplyType === 'vial_5ml' || hrtCfg?.supplyType === 'vial';
        if (hrtPickupQty > 0) {
          const todayStr = new Date().toISOString().split('T')[0];
          const { error: hrtSlErr } = await supabase.from('service_logs').insert({
            patient_id: patientId,
            protocol_id: protocolId,
            category: 'hrt',
            entry_type: 'pickup',
            ...
            medication: protocolData.medication || null,
            dosage: protocolData.selected_dose || null,
            quantity: hrtPickupQty,
            supply_type: hrtCfg.supplyType,
            fulfillment_method: fulfillmentMethod || 'in_clinic',
            tracking_number: trackingNumber || null,
            notes: `Auto-logged from HRT membership checkout: ${hrtPickupQty} pre-filled syring${hrtPickupQty === 1 ? 'e' : 'es'} dispensed (${hrtCfg.frequency || 'every 3.5 days'}).`,
          });
          if (hrtSlErr) console.error('Auto service_log error (hrt):', hrtSlErr);
          else console.log(`Auto-protocol: logged HRT pickup of ${hrtPickupQty} syringes for new protocol ${protocolId}`);
        } else if (hrtIsVialNew) {
          const todayStr = new Date().toISOString().split('T')[0];
          const hrtVialMl = hrtCfg.supplyType === 'vial_5ml' ? 5 : 10;
          const { error: hrtSlErr } = await supabase.from('service_logs').insert({
            patient_id: patientId,
          ...
          if (hrtSlErr) console.error('Auto service_log error (hrt vial):', hrtSlErr);
          else console.log(`Auto-protocol: logged HRT vial pickup (${hrtVialMl}ml) for new protocol ${protocolId}`);
        }
      }
    }
