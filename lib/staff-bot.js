...
// Cal.com is no longer in the loop. Slots, provider lookup, and booking
// creation use the native scheduling engine + createAppointment helper.
import { getAvailableSlots as engineGetAvailableSlots, pickProviderForSlot } from './scheduling';
...
async function findPatient(name, patientId) {
  // If a patient_id is provided, look up directly — no fuzzy matching needed
  if (patientId) {
    ...
    // Fall through to name search if ID didn't match
  }
...
// Match a fuzzy service name to a row in the local `services` table.
// Returns { id, slug, name (formerly 'title'), duration_minutes (formerly 'length') }
// keeping the old field names so downstream callers don't have to change.
async function findEventType(serviceName, location = null) {
  ...
  const { data: services, error } = await supabase
    .from('services')
    .select('id, slug, name, duration_minutes, group_label')
    .eq('is_active', true);
  ...
  // Return shape compatible with the old Cal.com event-type contract.
  return { id: pick.id, slug: pick.slug, title: pick.name, length: pick.duration_minutes };
}
...
async function handleBookAppointment(params) {
  try {
    ...
    const patient = await findPatient(params.patient_name, params.patient_id);
    if (!patient) {
    ...
    // Resolve provider — explicit if requested, otherwise round-robin
    // through pickProviderForSlot. providerName is the name we'll record on
    // the appointment row.
    ...
    if (params.provider_name) {
      ...
      if (match) {
        ...
        providerNote = `\n👩‍⚕️ Assigned to ${match.name}`;
      } else {
        providerNote = `\n⚠️ Provider "${params.provider_name}" not found — round-robin assigned instead`;
      }
    }
    if (!providerName) {
      const picked = await pickProviderForSlot({ serviceSlug: eventType.slug, startISO });
      if (!picked) {
        return `❌ No provider available for ${eventType.title} at ${formatDate(params.date)} ${formatTime(params.time)}.`;
      }
      providerName = picked.displayLabel || picked.name;
      if (!providerNote) providerNote = `\n👩‍⚕️ Assigned to ${providerName}`;
    }

    // Create the appointment via the shared helper (writes appointments,
    // shadow calcom_bookings, runs notifications + automations + audit).
    ...
    try {
      result = await createAppointment({
        ...
        visit_reason: `Staff assistant: ${eventType.title}`,
        source: 'staff_bot',
        created_by: 'staff_bot',
        send_notification: true,
      });
    } catch (e) {
      return `❌ Booking failed: ${e.message}\n\nDebug: serviceSlug=${eventType.slug}, start=${startISO}`;
    }
...
async function handleCreateTask(params, staff) {
  try {
    ...
    if (params.patient_name) {
      const p = await findPatient(params.patient_name, params.patient_id);
      if (p) {
        patientId = p.id;
...
async function handleLookupPatient(params) {
  try {
    // If patient_id is provided, do a direct lookup
    if (params.patient_id) {
      ...
      if (patient) {
        const dob = patient.date_of_birth
          ? new Date(patient.date_of_birth + 'T12:00:00Z').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'America/Los_Angeles' })
          : 'N/A';
    ...
    if (patients.length === 1) {
      ...
      const dob = p.date_of_birth
        ? new Date(p.date_of_birth + 'T12:00:00Z').toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
            year: 'numeric',
            timeZone: 'America/Los_Angeles',
...
async function handleCancelAppointment(params) {
  try {
    const { patient_name, patient_id, date, service_type, reason = 'Cancelled by staff' } = params;

    if (!patient_name && !patient_id) return '❌ Please provide the patient name.';

    // Read directly from the appointments table (single source of truth).
    let query = supabase
      .from('appointments')
      .select('id, patient_id, patient_name, patient_phone, start_time, end_time, duration_minutes, service_name, service_category, provider, location, status, cal_com_booking_id')
    ...
    await supabase.from('appointment_events').insert({
      ...
      metadata: { source: 'staff_bot', cancellation_reason: reason },
    });

    // 3. Patient notification (SMS + email).
    const { data: patientRow } = booking.patient_id
      ? await supabase.from('patients').select('email, phone').eq('id', booking.patient_id).single()
      : { data: null };
    ...
    sendAppointmentNotification({
      ...
      patient: {
        ...
        phone: patientRow?.phone || booking.patient_phone || null,
      },
      appointment: {
        serviceName,
    ...
    // 4. Provider SMS (replaces the legacy staff_email path).
    if (booking.provider) {
      try {
        const { sendProviderNotification } = await import('./provider-notifications');
        ...
        console.error('Provider notification import failed:', e);
      }
    }
    ...
    return `✅ Cancelled: **${serviceName}** for **${booking.patient_name}** on **${dt}**.\n\nNotifications sent to the patient${booking.provider ? ` and ${booking.provider}` : ''}.`;
  } catch (err) {
...
async function handleSendDocument(params) {
  try {
    const { patient_name, patient_id, document, method = 'sms' } = params;
    ...
    // Find patient
    const patient = await findPatient(patient_name, patient_id);
    if (!patient) {
...
async function handleSendForms(params) {
  try {
    const { patient_name, patient_id, bundle_type } = params;
...
function deriveCategory(protocol) {
  ...
  if (pt.includes('injection') || pt.includes('b12') || pt.includes('vitamin') || pt.includes('nad')) return 'Range Injection';
  if (pt.includes('lab')) return 'Labs';
  // Fall back to checking medication name
  const med = (protocol.medication || '').toLowerCase();
  if (med.includes('nad')) return 'Range Injection';
  if (med.includes('b12') || med.includes('lipo') || med.includes('taurine') || med.includes('toradol') || med.includes('glutathione')) return 'Range Injection';
...
async function handleGetPatientProtocols(params) {
  try {
    const { patient_name, patient_id, include_inactive } = params;
    ...
    const fmtDate = (d) => d ? new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' , timeZone: 'America/Los_Angeles' }) : null;
...
async function handleGetPatientAppointments(params) {
  try {
    const { patient_name, patient_id, include_past, limit: rawLimit } = params;
    if (!patient_name) return '❌ Patient name is required.';
    ...
    let query = supabase
      .from('appointments')
      .select('id, service_name, start_time, end_time, status, provider, location, notes')
      .eq('patient_id', patient.id)
      .not('status', 'eq', 'cancelled')
    ...
    for (const b of bookings) {
      ...
      if (b.provider) lines.push(`   Provider: ${b.provider}`);
...
async function handleRescheduleAppointment(params) {
  try {
    const { patient_name, patient_id, new_date, new_time, current_date, service_type } = params;
    ...
    let query = supabase
      .from('appointments')
      .select('id, patient_id, patient_name, patient_phone, service_name, service_category, provider, location, notes, start_time, end_time, duration_minutes, status, cal_com_booking_id, visit_group_id, source, created_by')
      .eq('patient_id', patient.id)
      .in('status', ['scheduled', 'confirmed'])
      .order('start_time', { ascending: true })
      .limit(10);
    ...
    await supabase.from('appointments').update({ status: 'rescheduled' }).eq('id', booking.id);
    ...
    await supabase.from('appointment_events').insert({
      ...
      metadata: { source: 'staff_bot', new_time: newStart },
    });

    // Create the new appointment row.
    const { data: newAppt, error: createErr } = await supabase
      ...
      .insert({
        ...
        visit_group_id: booking.visit_group_id || null,
      })
      .select()
      .single();
    ...
    const { data: patientRow } = await supabase
      .from('patients').select('email, phone').eq('id', patient.id).single();
...
async function handleLookupIntakeData({ patient_id }) {
  if (!patient_id) return { error: 'patient_id is required' };
  const { data, error } = await supabase
    .from('intakes')
    .select('first_name, last_name, submitted_at, has_allergies, allergies, allergy_reactions, on_medications, current_medications, medication_notes, on_hrt, hrt_details, medical_conditions, has_pcp, pcp_name, recent_hospitalization, hospitalization_reason, symptoms, symptom_followups, symptom_duration, additional_notes')
    .eq('patient_id', patient_id)
    .order('submitted_at', { ascending: false })
    .limit(1);
  if (error) return { error: error.message };
  if (!data || data.length === 0) return { has_intake: false, message: 'No intake form found for this patient.' };
  const i = data[0];
  return {
    has_intake: true,
    submitted_at: i.submitted_at,
    allergies: { has_allergies: i.has_allergies || 'Unknown', list: i.allergies || null, reactions: i.allergy_reactions || null },
    medications: { on_medications: i.on_medications || 'Unknown', current_list: i.current_medications || null, notes: i.medication_notes || null, on_hrt: i.on_hrt || 'Unknown', hrt_details: i.hrt_details || null },
    medical_conditions: i.medical_conditions || {},
    healthcare: { has_pcp: i.has_pcp || 'Unknown', pcp_name: i.pcp_name || null, recent_hospitalization: i.recent_hospitalization || 'Unknown', hospitalization_reason: i.hospitalization_reason || null },
    concerns: { symptoms: i.symptoms || [], duration: i.symptom_duration || null, additional_notes: i.additional_notes || null },
  };
}

async function handleLookupConsentStatus({ patient_id }) {
  if (!patient_id) return { error: 'patient_id is required' };
  const { data, error } = await supabase
    .from('consents')
    .select('consent_type, consent_date, consent_given, submitted_at, additional_data')
    .eq('patient_id', patient_id)
    .order('submitted_at', { ascending: false });
  if (error) return { error: error.message };
  ...
  const forms = signed.map(c => {
    const entry = { type: c.consent_type, signed_date: c.consent_date || c.submitted_at };
    if (c.additional_data?.healthAnswers) entry.health_screening = c.additional_data.healthAnswers;
    return entry;
  });
  return {
    forms,
