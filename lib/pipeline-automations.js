...
async function getConsultAppointmentState(client, patientId, since) {
...
export async function runStageEntry({ card, stage, context = {} }) {
  ...
  if (stage === 'ready_to_schedule') {
    ...
    const apptState = await getConsultAppointmentState(client, card.patient_id, card.created_at);
    if (apptState) {
...
export async function ensureLabsCardAtUnderReview({
  ...
  let existing = await findActiveCard({ patient_id: patientId, pipeline: 'energy_workup' });
  if (!existing) {
    existing = await findActiveCard({ patient_id: patientId, pipeline: 'follow_up_labs' });
  }
...
/**
 * Ensure the patient has a labs-pipeline card at `labs_scheduled`.
 * Called when a blood draw appointment is booked — creates the card if no
 * active card exists, otherwise leaves the existing card alone (never moves
 * backwards).
 */
export async function ensureLabsCardAtLabsScheduled({
  patientId,
  labType = 'Follow-Up',
  reason = 'blood_draw_appointment_booked',
}) {
  if (!patientId) return null;

  const { findActiveCard, createCard } = await import('./pipelines-server');

  const existing = await findActiveCard({ patient_id: patientId, pipeline: 'energy_workup' });
  if (existing) return existing;

  return await createCard({
    pipeline: 'energy_workup',
    stage: 'labs_scheduled',
    patient_id: patientId,
    meta: { lab_type: labType },
    triggered_by: 'automation',
    automation_reason: reason,
  });
}
...
/**
 * Advance the labs pipeline when an appointment is completed.
 * Blood draw → labs_scheduled → awaiting_results.
 * Consult/lab review → pre-completed stage → consult_completed.
 * Shared by both /api/appointments/update and /api/appointments/[id]/status.
 */
export async function advanceLabsPipelineOnCompletion(appointment) {
  ...
  if (card.stage === 'labs_scheduled' && isBloodDraw) {
    await moveCard({
      card_id: card.id,
      to_stage: 'awaiting_results',
      triggered_by: 'automation',
