...
import { getProviderSignatureBytes } from './provider-signature';
...
export async function generateMedicationListPdf({
  ...
  const drawMedication = (med, index) => {
    ...
    if (med.dispense) {
      yPos -= 2;
      drawWrappedLine(`Disp: ${med.dispense}`, {
        x: LEFT_MARGIN, size: 10, maxWidth: CONTENT_WIDTH, lineHeight: 14, bold: true, color: COLORS.darkGray,
      });
    }
  ...
  const drawPrescriberBlock = async () => {
    ...
    try {
      const sigBytes = getProviderSignatureBytes();
      const sigImage = await pdfDoc.embedPng(sigBytes);
      const sigDisplayHeight = 42;
      const scale = sigDisplayHeight / sigImage.height;
      const sigDisplayWidth = sigImage.width * scale;
      page.drawImage(sigImage, {
        x: LEFT_MARGIN,
        y: yPos - sigDisplayHeight + 6,
    ...
    yPos -= sigEmbedded ? 46 : 4;
    page.drawLine({
      start: { x: LEFT_MARGIN, y: yPos + 2 },
      end: { x: LEFT_MARGIN + 240, y: yPos + 2 },
      thickness: 0.5, color: COLORS.darkGray,
    });
    yPos -= 18;

    safeDraw('Damien Burgess', {
      x: LEFT_MARGIN, y: yPos, size: 11, font: fontBold, color: COLORS.black,
    });
    yPos -= 13;
    safeDraw('Range Medical', {
      x: LEFT_MARGIN, y: yPos, size: 9.5, font, color: COLORS.textBody,
    });
    yPos -= 14;

    const credLine = 'NPI: 1699917138    State License #: 95014074    DEA: MB9640586';
    safeDraw(credLine, {
      x: LEFT_MARGIN, y: yPos, size: 9, font, color: COLORS.darkGray,
    });
    yPos -= 14;
  };
  ...
  infoRows.push({ label: 'Prescribing Provider', value: provider || 'Damien Burgess' });
