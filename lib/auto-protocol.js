...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}, fulfillmentOpts = {}) {
  ...
  switch (protocolType) {
    ...
    case 'hrt': {
      // Read the patient's gender from opts (caller looks it up before calling
      // this builder). Fall back to 'male' only if gender is missing — the
      // provider review task will catch any miscategorization.
      const gender = (opts.patientGender || 'male').toLowerCase();
      const isFemale = gender === 'female';
      const hrtEnd = new Date();
      hrtEnd.setDate(hrtEnd.getDate() + 30);

      // Sensible defaults — the provider verifies and adjusts via the
      // "Needs Provider Review" flag on the patient page.
      const dose = isFemale ? '0.25ml/25mg' : '0.5ml/100mg';
      const frequency = isFemale ? 'Weekly' : 'Every 3.5 days';
      const sig = buildSig({
        dose,
        route: 'Intramuscular',
        frequency,
      ...
      return {
        ...
        medication: getHRTMedication(gender),
        hrt_type: isFemale ? 'female' : 'male',
        selected_dose: dose,
        starting_dose: dose,
        current_dose: dose,
        frequency: 'every_3_5_days', // legacy enum string preserved for downstream consumers
        injection_method: 'im',
        sig,
        delivery_method: 'take_home',
        end_date: hrtEnd.toISOString().split('T')[0],
        // Auto-created from purchase — the provider must verify before dispense.
        needs_medication_review: true,
      };
    }
...
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId, wlFrequencyDays, wlConfig, peptideConfig, injectionFrequency }) {
  try {
    ...
    if (existing) {
      ...
      // Create new protocol
      const protocolData = buildProtocolData(protocolType, serviceName, patientId, { deliveryMethod, durationDays, medication, patientName, patientGender, serviceCategory, quantity, wlFrequencyDays, wlConfig, peptideConfig, injectionFrequency }, { fulfillmentMethod });
