...
// "N-day". Falls back to 7 for unrecognized strings so weekly stays the safe
// default. Used by the patient detail UI and the check-in reminder cron to
// keep injection projections and SMS cadence aligned with the protocol.
export const parseFrequencyDays = (frequency) => {
  if (!frequency) return 7;
  const lower = String(frequency).toLowerCase().trim();
  const everyMatch = lower.match(/every\s+(\d+)\s*day/);
  if (everyMatch) {
  ...
  if (lower.includes('monthly')) return 28;
...
export const INJECTION_PRICING = {
  ...
  premium: {
    ...
    items: ['L-Carnitine', 'Glutathione (200mg)', 'MIC-B12 (Skinny Shot)'],
  },
  nad: {
