...
export const NEEDLE_GAUGES = ['18G', '20G', '22G', '23G', '25G', '27G', '30G', '32G'];
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { key: 'other_medication', label: 'Medication Name', type: 'text', placeholder: 'Enter medication name', required: true, conditionalOn: { field: 'medication_name', value: 'Other' } },
      ...
      },
      {
        ...
        key: 'weight_vitals',
        fields: [
          { key: 'current_weight', label: 'Current Weight (lbs)', type: 'text', placeholder: 'e.g. 185' },
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'other_side_effect', label: 'Other Side Effect', type: 'text', placeholder: 'Describe side effect...', conditionalOn: { field: 'reported_effects', value: 'Other' } },
      ...
      },
      {
        ...
        key: 'addons',
        fields: [
          { key: 'addon_injections', label: 'Add-On Vitamins / Injections', type: 'vitamin_dose', items: [
            { name: 'B12 (Methylcobalamin)',   mgPerMl: 10,   maxCc: 1,  strengthLabel: '10 mg/mL' },
            { name: 'B12 (Cyanocobalamin)',     mgPerMl: 10,   maxCc: 1,  strengthLabel: '10 mg/mL' },
            { name: 'Skinny + Injection', mgPerMl: null, maxCc: 2,  compound: true, strengthLabel: 'MIC + L-Carnitine + B12' },
            { name: 'Skinny Shot',      mgPerMl: null, maxCc: 2,  compound: true, strengthLabel: 'MIC + B12' },
            { name: 'Glutathione',              mgPerMl: 200,  maxCc: 3,  strengthLabel: '200 mg/mL' },
            { name: 'Biotin',                   mgPerMl: 5,    maxCc: 2,  strengthLabel: '5 mg/mL' },
            { name: 'Vitamin D3',               iuPerMl: 50000, mgPerMl: null, maxCc: 1,  strengthLabel: '50,000 IU/mL' },
          ] },
          { key: 'addon_site', label: 'Add-On Injection Site', type: 'button_group', options: INJECTION_SITES, showIf: (data) => (data?.addons?.addon_injections || []).length > 0 },
          { key: 'addon_notes', label: 'Add-On Notes', type: 'text', placeholder: 'e.g. Patient requested B12 for energy', showIf: (data) => (data?.addons?.addon_injections || []).length > 0 },
        ],
      },
      {
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'follow_up', label: 'Follow Up', type: 'select', options: [
            ...
            '2 weeks — reassess',
            '4 weeks — reassess',
            '6 weeks — labs/reassess',
            '8 weeks — labs/reassess',
            '12 weeks — labs/reassess',
            'PRN',
          ], defaultValue: 'Next scheduled injection' },
        ],
      },
      {
        title: 'Additional Notes',
    ...
    sections: [
      {
        ...
        key: 'iv_placement',
        fields: [
          { key: 'placement_date', label: 'Placement Date', type: 'text', placeholder: 'MM/DD/YYYY', defaultValue: new Date().toLocaleDateString('en-US'), required: true },
          { key: 'placement_time', label: 'Placement Time', type: 'text', placeholder: 'e.g. 10:30 AM' },
          { key: 'placed_by', label: 'Placed By', type: 'text', placeholder: 'Name of person placing IV' },
        ],
      },
      {
        title: 'IV Access Details',
        type: 'fields',
        key: 'iv_access',
        fields: [
          { key: 'location', label: 'Location', type: 'button_group', options: [
            'Antecubital', 'Arm', 'External Jugular', 'Foot', 'Forearm', 'Hand', 'Leg', 'Saphenous', 'Scalp', 'Wrist',
          ], required: true },
          { key: 'orientation', label: 'Orientation', type: 'multi_check', options: [
            'Right', 'Left', 'Anterior', 'Posterior', 'Lateral', 'Medial', 'Proximal', 'Distal', 'Upper', 'Lower',
          ], required: true },
          { key: 'catheter_length', label: 'IV Catheter Length', type: 'button_group', options: [
            '0.75 in', '1 in', '1.25 in', '1.75 in', '1.88 in', '2.25 in', '2.63 in',
          ] },
          { key: 'catheter_size', label: 'Size (Gauge)', type: 'button_group', options: [
            '20G', '22G', '24G', '25G', '30G',
          ], required: true },
          { key: 'ultrasound_guidance', label: 'Ultrasound Guidance Used?', type: 'toggle', options: ['Yes', 'No'], defaultValue: 'No' },
          { key: 'deep_vein_iv', label: 'Deep Vein IV?', type: 'toggle', options: ['Yes', 'No'], defaultValue: 'No' },
          { key: 'site_prep', label: 'Site Prep', type: 'multi_check', options: [
            'Alcohol', 'Betadine', 'Chlorhexidine', 'Other (Comment)',
          ], required: true },
          { key: 'local_anesthetic', label: 'Local Anesthetic', type: 'button_group', options: ['Injectable', 'Topical', 'None'] },
          { key: 'attempts', label: 'Insertion Attempts', type: 'button_group', options: ['1', '2', '3', '4', '5+', 'Other'], required: true },
          { key: 'iv_tolerance', label: 'Patient Tolerance', type: 'button_group', options: ['Tolerated well', 'Tolerated poorly', 'Other (Comment)'], required: true },
          { key: 'iv_tolerance_comment', label: 'Tolerance Comment', type: 'textarea', placeholder: 'Describe patient tolerance...', conditionalOn: { field: 'iv_tolerance', value: 'Other (Comment)' } },
          { key: 'difficult_access', label: 'Difficult Venous Access', type: 'toggle', options: ['Yes', 'No'], defaultValue: 'No' },
      ...
      },
      {
        ...
        fields: [
          { key: 'infusion_type', label: 'Infusion Type', type: 'select', options: [
            ...
            'High-Dose Vitamin C 50g',
            'High-Dose Vitamin C 75g',
            'Methylene Blue IV',
            'MB + Vit C + Mag Combo',
            'Glutathione IV 1g',
            'Glutathione IV 2g',
          ...
          { key: 'fluid_type', label: 'IV Fluid', type: 'button_group', options: ['Normal Saline (0.9% NaCl)', 'Dextrose 5% (D5W)'], required: true },
          { key: 'saline_volume', label: 'Fluid Volume (mL)', type: 'button_group', options: ['100', '250', '500', '1000'], required: true },
          { key: 'nad_dose', label: 'NAD+ Dose (mg)', type: 'button_group', options: ['250', '500', '750', '1000'] },
          { key: 'vitamins', label: 'Vitamins / Additives', type: 'vitamin_dose', required: true, items: [
            { name: 'Vitamin C (Ascorbic Acid)',      mgPerMl: 500,   maxCc: 10, supplier: 'AnazaoHealth', strengthLabel: '500 mg/mL' },
            { name: 'B-Complex (B1, B2, B3, B5, B6)', mgPerMl: null,  maxCc: 3,  supplier: 'Olympia/THS', compound: true, strengthLabel: 'B1 100, B3 100, B2 2, B6 2, B5 2 mg/mL' },
            { name: 'Vitamin B12 (Methylcobalamin)',   mgPerMl: 10,    maxCc: 3,  supplier: 'AnazaoHealth', unit: 'mg', note: '10,000 mcg/mL', strengthLabel: '10,000 mcg/mL' },
            { name: 'Magnesium Chloride',              mgPerMl: 200,   maxCc: 5,  supplier: 'McGuff', strengthLabel: '200 mg/mL' },
            { name: 'Calcium Gluconate',               mgPerMl: 100,   maxCc: 5,  strengthLabel: '100 mg/mL' },
            { name: 'Zinc Sulfate',                    mgPerMl: 10,    maxCc: 3,  supplier: 'THS', strengthLabel: '10 mg/mL' },
            { name: 'Selenium',                        mgPerMl: 0.2,   maxCc: 3,  unit: 'mcg', mcgPerMl: 200, strengthLabel: '200 mcg/mL' },
            { name: 'Glutathione',                     mgPerMl: 200,   maxCc: 15, supplier: 'AnazaoHealth', strengthLabel: '200 mg/mL' },
            { name: 'Biotin',                          mgPerMl: 10,    maxCc: 3,  strengthLabel: '10 mg/mL' },
            { name: 'Taurine',                         mgPerMl: 50,    maxCc: 5,  strengthLabel: '50 mg/mL' },
            { name: 'L-Carnitine',                     mgPerMl: 500,   maxCc: 5,  supplier: 'Olympia/THS', strengthLabel: '500 mg/mL' },
            { name: 'Alpha Lipoic Acid (ALA)',         mgPerMl: 25,    maxCc: 5,  strengthLabel: '25 mg/mL' },
            { name: 'Dexpanthenol (B5)',               mgPerMl: 250,   maxCc: 3,  strengthLabel: '250 mg/mL' },
            { name: 'Mineral Blend (Mg/Zn/Mn/Cu)',     mgPerMl: null,  maxCc: 5,  supplier: 'THS', compound: true, strengthLabel: 'Mg 80/Zn 1/Mn 20/Cu 0.2 mg/mL' },
            { name: 'Tri-Immune (Glut/C/Zinc)',        mgPerMl: null,  maxCc: 5,  supplier: 'THS', compound: true, strengthLabel: 'Glutathione/Ascorbic Acid/Zinc' },
            { name: 'Amino Blend (AminoMultiPlex)',    mgPerMl: null,  maxCc: 5,  supplier: 'AnazaoHealth', compound: true, strengthLabel: 'Arg 50/Glut 15/Lys 15/Pro 25 mg/mL' },
            { name: 'NAC (Acetylcysteine)',             mgPerMl: 200,   maxCc: 5,  supplier: 'THS', strengthLabel: '200 mg/mL' },
            { name: 'NAD+',                            mgPerMl: 125,   maxCc: 10, supplier: 'AnazaoHealth', strengthLabel: '125 mg/mL' },
            { name: 'Thiamine/Pyridoxine (B1/B6)',     mgPerMl: null,  maxCc: 3,  compound: true, strengthLabel: 'B1 20/B6 100 mg/mL' },
            { name: 'Methylene Blue',                  mgPerMl: 5,     maxCc: 5,  supplier: 'ALD', strengthLabel: '5 mg/mL' },
          ] },
          { key: 'glutathione', label: 'Glutathione IV Push', type: 'select', options: [
            ...
            '1g — slow IV push at completion',
            '2g — slow IV push at completion',
            '3g — slow IV push at completion',
          ], defaultValue: 'None' },
          ...
          { key: 'additional_meds', label: 'Additional Notes', type: 'textarea', placeholder: 'e.g. extra Vitamin C, special instructions, etc.' },
        ],
      },
      {
        title: 'IM Injection Add-On',
        type: 'fields',
        key: 'im_addon',
        fields: [
          { key: 'im_injections', label: 'IM Injections', type: 'vitamin_dose', items: [
            { name: 'B12 (Methylcobalamin)',         mgPerMl: 10,    maxCc: 1,  strengthLabel: '10 mg/mL' },
            { name: 'B12 (Cyanocobalamin)',           mgPerMl: 10,    maxCc: 1,  strengthLabel: '10 mg/mL' },
            { name: 'Thiamine/Pyridoxine (B1/B6)',    mgPerMl: null,  maxCc: 2,  compound: true, strengthLabel: 'B1 20/B6 100 mg/mL' },
            { name: 'Vitamin D3',                     iuPerMl: 50000, mgPerMl: null, maxCc: 1,  strengthLabel: '50,000 IU/mL' },
            { name: 'Skinny + Injection',      mgPerMl: null,  maxCc: 2,  compound: true, strengthLabel: 'MIC + L-Carnitine + B12' },
            { name: 'Skinny Shot',            mgPerMl: null,  maxCc: 2,  compound: true, strengthLabel: 'MIC + B12' },
            { name: 'Glutathione',                    mgPerMl: 200,   maxCc: 3,  strengthLabel: '200 mg/mL' },
            { name: 'Biotin',                         mgPerMl: 5,     maxCc: 2,  strengthLabel: '5 mg/mL' },
            { name: 'L-Carnitine',                    mgPerMl: 500,   maxCc: 2,  strengthLabel: '500 mg/mL' },
            { name: 'Toradol (Ketorolac)',            mgPerMl: 30,    maxCc: 1,  strengthLabel: '30 mg/mL' },
          ] },
          { key: 'im_site', label: 'IM Injection Site', type: 'button_group', options: INJECTION_SITES, showIf: (data) => (data?.im_addon?.im_injections || []).length > 0 },
          { key: 'im_notes', label: 'IM Injection Notes', type: 'text', placeholder: 'e.g. Patient requested B12 for energy', showIf: (data) => (data?.im_addon?.im_injections || []).length > 0 },
        ],
      },
      {
        title: 'Vitals',
      ...
      },
      {
        ...
        key: 'iv_removal',
        fields: [
          { key: 'removal_date', label: 'Removal Date', type: 'text', placeholder: 'MM/DD/YYYY' },
          { key: 'removal_time', label: 'Removal Time', type: 'text', placeholder: 'e.g. 11:45 AM' },
          { key: 'removal_reason', label: 'Removal Reason', type: 'multi_check', options: [
            'Therapy completed',
            'Per protocol',
            'Per order',
            ...
            'Removed by patient',
            'Other (Comment)',
          ], defaultChecked: [0], required: true },
          { key: 'post_removal_assessment', label: 'Post-Removal Site Assessment', type: 'multi_check', options: [
            'WDL — No signs of complication',
            'Bleeding at site',
            'Bruising at site',
            'Drainage',
            'Painful',
  ...
  injection: {
    ...
    sections: [
      ...
      },
      {
        ...
        key: 'medication',
        repeatable: true,
        repeatLabel: 'Add Another Injection',
        fields: [
          { key: 'medication_name', label: 'Medication', type: 'select', options: [
          ...
          { key: 'nad_dose', label: 'Dose', type: 'select', options: ['25mg', '50mg', '75mg', '100mg', '125mg', '150mg', '175mg', '200mg'], required: true, conditionalOn: { field: 'medication_name', value: 'NAD+' } },
          { key: 'dose', label: 'Dose', type: 'text', placeholder: 'e.g. 1mL, 1000mcg, 30mg', required: true, conditionalOnNot: { field: 'medication_name', value: 'NAD+' } },
          { key: 'route', label: 'Route', type: 'select', options: INJECTION_ROUTES, required: true },
          { key: 'site', label: 'Injection Site', type: 'button_group', options: INJECTION_SITES, required: true },
          { key: 'needle_gauge', label: 'Needle Gauge', type: 'button_group', options: NEEDLE_GAUGES },
...
function generateWeightLossNote(data, vitals) {
  ...
  // Medication
  const medName = med.medication_name === 'Other' && med.other_medication ? `Other — ${med.other_medication}` : med.medication_name;
  if (medName) lines.push(`**Medication:** ${medName}${med.pharmacy ? ` (${med.pharmacy})` : ''}`);
  if (med.dose) lines.push(`**Dose:** ${med.dose}`);
  if (med.route) lines.push(`**Route:** ${med.route}`);
  ...
  if (effects.length === 1 && effects[0] === 'None') {
    ...
    const filtered = effects.filter(e => e !== 'None').map(e => {
      if (e === 'Other' && se.other_side_effect) return `Other — ${se.other_side_effect}`;
      return e;
    });
  ...
  const addonList = addons.addon_injections || [];
  const addonDoses = addons.vitamin_doses || {};
  if (addonList.length > 0) {
    lines.push('**Add-On Injections:**');
    addonList.forEach(v => {
      const dose = addonDoses[v];
      if (dose && dose.cc) {
        const doseLabel = dose.iu ? `${dose.iu.toLocaleString()} IU` : dose.mcg ? `${dose.mcg} mcg` : dose.mg ? `${dose.mg} mg` : null;
        lines.push(`- ${v} — ${dose.cc} mL${doseLabel ? ` (${doseLabel})` : ''}`);
      } else {
        lines.push(`- ${v}`);
      }
    });
    if (addons.addon_site) lines.push(`**Add-On Site:** ${addons.addon_site}`);
    if (addons.addon_notes) lines.push(`**Add-On Notes:** ${addons.addon_notes}`);
    lines.push('');
  }
...
function generateIVNote(data) {
  const lines = [];
  const placement = data.iv_placement || {};
  const access = data.iv_access || {};
  const infusion = data.infusion || {};
  const vitalsData = data.vitals || {};
  const outcome = data.outcome || {};
  const removal = data.iv_removal || {};
  const additional = data.additional || {};

  // IV Placement
  lines.push('**IV Placement**');
  if (placement.placement_date || placement.placement_time) {
    const parts = [];
    if (placement.placement_date) parts.push(placement.placement_date);
    if (placement.placement_time) parts.push(placement.placement_time);
    lines.push(`**Placed:** ${parts.join(' at ')}`);
  }
  if (placement.placed_by) lines.push(`**Placed By:** ${placement.placed_by}`);
  ...
  if (access.location) {
    const orientationArr = access.orientation || [];
    const orientationStr = orientationArr.length > 0 ? ` (${orientationArr.join(', ')})` : '';
    lines.push(`**Location:** ${access.location}${orientationStr}`);
  }
  if (access.catheter_size) lines.push(`**Catheter Size:** ${access.catheter_size}`);
  if (access.catheter_length) lines.push(`**Catheter Length:** ${access.catheter_length}`);
  if (access.ultrasound_guidance === 'Yes') lines.push('**Ultrasound Guidance:** Yes');
  if (access.deep_vein_iv === 'Yes') lines.push('**Deep Vein IV:** Yes');
  const sitePrep = access.site_prep || [];
  if (sitePrep.length > 0) lines.push(`**Site Prep:** ${sitePrep.join(', ')}`);
  if (access.local_anesthetic && access.local_anesthetic !== 'None') lines.push(`**Local Anesthetic:** ${access.local_anesthetic}`);
  if (access.attempts) lines.push(`**Insertion Attempts:** ${access.attempts}`);
  if (access.difficult_access === 'Yes') lines.push('**Difficult Venous Access:** Yes');
  if (access.iv_tolerance) {
    let tolStr = access.iv_tolerance;
    if (access.iv_tolerance === 'Other (Comment)' && access.iv_tolerance_comment) {
      tolStr = access.iv_tolerance_comment;
    }
    lines.push(`**Patient Tolerance to IV Start:** ${tolStr}`);
  }
  ...
  if (infusion.infusion_type) {
    ...
    const fluidName = infusion.fluid_type || 'Normal Saline (0.9% NaCl)';
    if (infusion.saline_volume) lines.push(`Prepared and administered in ${infusion.saline_volume} mL of ${fluidName}.`);
  }

  // Vitamins / Additives
  const vitamins = infusion.vitamins || [];
  const vitaminDoses = infusion.vitamin_doses || {};
  if (vitamins.length > 0) {
    lines.push('');
    lines.push('**Vitamins / Additives:**');
    vitamins.forEach(v => {
      const dose = vitaminDoses[v];
      if (dose && dose.cc) {
        const doseLabel = dose.iu ? `${dose.iu.toLocaleString()} IU` : dose.mcg ? `${dose.mcg} mcg` : dose.mg ? `${dose.mg} mg` : null;
        lines.push(`- ${v} — ${dose.cc} mL${doseLabel ? ` (${doseLabel})` : ''}`);
      } else {
  ...
  // IM Injection Add-Ons
  const imAddon = data.im_addon || {};
  const imInjections = imAddon.im_injections || [];
  const imDoses = imAddon.vitamin_doses || {};
  if (imInjections.length > 0) {
    lines.push('**IM Injection Add-On:**');
    imInjections.forEach(v => {
      const dose = imDoses[v];
      if (dose && dose.cc) {
        const doseLabel = dose.iu ? `${dose.iu.toLocaleString()} IU` : dose.mcg ? `${dose.mcg} mcg` : dose.mg ? `${dose.mg} mg` : null;
        lines.push(`- ${v} — ${dose.cc} mL${doseLabel ? ` (${doseLabel})` : ''}`);
      } else {
    ...
    if (imAddon.im_site) lines.push(`**IM Site:** ${imAddon.im_site}`);
    if (imAddon.im_notes) lines.push(`**IM Notes:** ${imAddon.im_notes}`);
    lines.push('');
  }
  ...
  // IV Removal
  if (removal.removal_date || removal.removal_time || (removal.removal_reason && removal.removal_reason.length > 0)) {
    lines.push('');
    lines.push('**IV Removal**');
    if (removal.removal_date || removal.removal_time) {
      const parts = [];
      if (removal.removal_date) parts.push(removal.removal_date);
      if (removal.removal_time) parts.push(removal.removal_time);
      lines.push(`**Removed:** ${parts.join(' at ')}`);
    }
    const reasons = removal.removal_reason || [];
    if (reasons.length > 0) {
      lines.push(`**Removal Reason:** ${reasons.join(', ')}`);
    }
    const postRemoval = removal.post_removal_assessment || [];
    if (postRemoval.length > 0) {
      lines.push(`**Post-Removal Assessment:** ${postRemoval.join(', ')}`);
    }
  }
...
function generateInjectionNote(data) {
  ...
  // Collect all medication entries (supports multiple injections)
  const meds = [];
  // Check for repeatable medication entries (medication_0, medication_1, ...)
  for (let i = 0; i < 20; i++) {
    const entry = data[`medication_${i}`];
    if (entry && entry.medication_name) meds.push(entry);
  }
  // Fallback: single medication entry (legacy format)
  if (meds.length === 0 && data.medication?.medication_name) {
    meds.push(data.medication);
  }

  meds.forEach((med, idx) => {
    if (meds.length > 1) {
      lines.push(`**--- Injection ${idx + 1} ---**`);
    }
    if (med.medication_name) lines.push(`**Medication:** ${med.medication_name}${med.pharmacy ? ` (${med.pharmacy})` : ''}`);
    const injDose = med.nad_dose || med.dose;
    if (injDose) lines.push(`**Dose:** ${injDose}`);
    if (med.route) lines.push(`**Route:** ${med.route}`);
    if (med.site) lines.push(`**Site:** ${med.site}`);
    if (med.needle_gauge) lines.push(`**Needle:** ${med.needle_gauge}`);
