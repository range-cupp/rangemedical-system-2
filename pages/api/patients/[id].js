...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const consentFields = 'id, consent_type, first_name, last_name, email, phone, consent_date, consent_given, signature_url, pdf_url, submitted_at, patient_id, ghl_contact_id, additional_data';
      ...
      const [
        ...
        supabase.from('service_logs').select('id, protocol_id, entry_type, entry_date, medication, dosage, weight, quantity, notes, fulfillment_method, tracking_number, slot_fulfillment').eq('patient_id', id).eq('category', 'weight_loss').order('entry_date', { ascending: true }),
        // All service logs
      ...
      // ── LTV calculation ──
      // Always use amount_paid — this is the per-item amount the patient paid.
      // Do NOT use stripe_amount_cents — that field may contain the cart total
      // (not per-item) for multi-item transactions.
