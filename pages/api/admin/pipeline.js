...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch weight logs for weight_loss protocols (safely - column might not exist yet)
    let weightDataByProtocol = {};
    try {
      ...
      Object.keys(weightDataByProtocol).forEach(key => {
        ...
        weightDataByProtocol[key].starting_weight = weights[weights.length - 1];
      });
    } catch (weightError) {
      console.log('Weight data fetch skipped (column may not exist yet):', weightError.message);
    }
