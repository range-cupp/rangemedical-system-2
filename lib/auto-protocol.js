...
function buildProtocolData(protocolType, serviceName, patientId, opts = {}, fulfillmentOpts = {}) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      const frequencyDays = parseWlFrequency(serviceName, opts.wlFrequencyDays);
...
// ================================================================
async function extendProtocol(protocol, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays } = {}) {
  ...
  switch (protocolType) {
    case 'weight_loss': {
      ...
      const wlFreqDays = parseWlFrequency(serviceName, wlFrequencyDays);
      ...
      // Update frequency on protocol if it changed
...
// ================================================================
export async function autoCreateOrExtendProtocol({ patientId, serviceCategory, serviceName, purchaseId, deliveryMethod, durationDays, quantity, fulfillmentMethod, trackingNumber, stripePaymentIntentId, wlFrequencyDays }) {
  try {
    ...
    if (existing) {
      ...
      protocolId = await extendProtocol(existing, protocolType, serviceName, { fulfillmentMethod, trackingNumber, quantity, wlFrequencyDays });
      console.log(`Auto-protocol: extended ${protocolType} protocol ${protocolId} for patient ${patientId}`);
    } else {
      // Create new protocol
      const protocolData = buildProtocolData(protocolType, serviceName, patientId, { deliveryMethod, durationDays, medication, patientName, serviceCategory, quantity, wlFrequencyDays }, { fulfillmentMethod });
      ...
      if (protocolType === 'weight_loss') {
        ...
        if (fulfillmentMethod === 'in_clinic_injections') {
          ...
          const newWlFreqDays = parseWlFrequency(serviceName, wlFrequencyDays);
