...
/**
 * Push to every employee that has a registered subscription.
 * Used for global events like an inbound patient SMS — any staff member
 * with notifications enabled should be alerted.
 *
 * @param {object} payload  { title, body, data: { kind, patient_id, ... } }
 */
export async function pushAllEmployees(payload) {
  const { data, error } = await supabase
    .from('push_subscriptions')
    .select('employee_id');

  if (error) {
    console.warn('[web-push] fetch all subs failed:', error.message);
    return;
  }

  const ids = [...new Set((data || []).map((r) => r.employee_id).filter(Boolean))];
  if (ids.length === 0) return;
  return pushToEmployees(ids, payload);
}
