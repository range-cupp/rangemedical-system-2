...
function deriveCategory(protocol) {
  ...
  if (pt.includes('iv') || pt.includes('infusion') || pt.includes('nad')) return 'IV';
  ...
  if (pt.includes('injection') || pt.includes('b12') || pt.includes('vitamin')) return 'Injection';
...
async function handleGetPatientProtocols(params) {
  try {
    ...
    let query = supabase
      .from('protocols')
      .select('id, program_type, program_name, medication, category, status, start_date, end_date, dosing, current_dose, selected_dose, frequency, injection_schedule, sessions_total, sessions_completed, sessions_used, total_sessions, next_lab_date, notes, indication, supply_type')
      .eq('patient_id', patient.id)
