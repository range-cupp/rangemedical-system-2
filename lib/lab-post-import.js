...
export async function postImportActions(supabase, patientId, patientName, testDate, reviewerIds) {
  ...
  // ── 5. Main Pipeline card + single joint review task ──────────────────────
  // First ensure the energy_workup card is at under_review (skip generic task
  // creation — we insert the rich joint task below instead).
  let pipelineCardId = null;
  try {
    const { ensureEnergyWorkupAtUnderReview } = await import('./pipeline-automations');
    const card = await ensureEnergyWorkupAtUnderReview({
      patientId,
