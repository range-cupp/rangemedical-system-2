...
// Exclude clearly non-consultation encounter types (procedures, pickups, injections, IVs, blood draws).
// Everything else from a provider (including "other", "general", null) gets a summary.
const NON_CONSULT_PATTERN = /\binjection\b|iv.?therapy|range.?iv|nad\+?\s*iv|blood.?draw|medication.?pickup|pellet.?procedure|hrt.?followup/i;
...
export function isProviderConsultNote(createdBy, encounterService) {
  ...
  // If no encounter service, still generate (providers sometimes skip tagging)
  if (!encounterService) return true;
  // Exclude procedural/non-consultation types
  return !NON_CONSULT_PATTERN.test(encounterService);
}
...
export function buildPatientPlanEmailHtml({ patientFirstName, providerName, noteDate, visitType, assessment, treatmentPlan, followUp, personalMessage }) {
  const dateStr = new Date(noteDate).toLocaleDateString('en-US', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
    timeZone: 'America/Los_Angeles',
  });
  ...
  const messageBlock = personalMessage
    ? `<p style="margin:0 0 16px;font-size:14px;line-height:1.6;color:#1f2937;">${personalMessage}</p>`
